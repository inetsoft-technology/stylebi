...
public class TwilioQuery extends EndpointJsonQuery<TwilioEndpoint> {
   public TwilioQuery() {
      super(TwilioDataSource.TYPE);
      setJsonPath("$");
   }
   ...
   @Override
   public Map<String, TwilioEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(TwilioEndpoints.class);
      }
