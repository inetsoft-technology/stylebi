...
public class LiveAgentQuery extends EndpointJsonQuery<LiveAgentEndpoint> {
   public LiveAgentQuery() {
      super(LiveAgentDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(LiveAgentEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.TOTAL_COUNT_AND_PAGE)
            .totalCountParam(PaginationParamType.HEADER, "X-Total-Count")
            .pageNumberParamToWrite(PaginationParamType.JSON_PATH, "_page")
            .maxResultsPerPageParam(PaginationParamType.QUERY, "_perPage")
   ...
   @Override
   public Map<String, LiveAgentEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(LiveAgentEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private Map<String, LiveAgentEndpoint> endpoints =
         Endpoints.load(LiveAgentEndpoints.class);
   }
}
