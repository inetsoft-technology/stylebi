...
public class WordPressQuery extends EndpointJsonQuery<WordPressEndpoint> {
   public WordPressQuery() {
      super(WordPressDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(WordPressEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, WordPressEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(WordPressEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, WordPressEndpoint> endpoints =
         Endpoints.load(WordPressEndpoints.class);
    }
}
