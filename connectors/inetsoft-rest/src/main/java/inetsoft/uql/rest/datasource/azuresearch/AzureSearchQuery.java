...
public class AzureSearchQuery extends EndpointJsonQuery<AzureSearchEndpoint> {
   public AzureSearchQuery() {
      super(AzureSearchDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(AzureSearchEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.LINK_ITERATION)
            .linkParam(PaginationParamType.JSON_PATH, "$['@odata.nexLink']")
            .build();
      }
      else {
         paginationSpec = PaginationSpec.builder()
   ...
   @Override
   public Map<String, AzureSearchEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(AzureSearchEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, AzureSearchEndpoint> endpoints =
         Endpoints.load(AzureSearchEndpoints.class);
   }
}
