...
public class ConstantContactQuery extends EndpointJsonQuery<ConstantContactEndpoint> {
   public ConstantContactQuery() {
      super(ConstantContactDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(ConstantContactEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.LINK_ITERATION)
            .linkParam(PaginationParamType.JSON_PATH, "$._links.next.href")
            .build();
      }
      else {
         paginationSpec = PaginationSpec.builder()
   ...
   @Override
   public Map<String, ConstantContactEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(ConstantContactEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, ConstantContactEndpoint> endpoints =
         Endpoints.load(ConstantContactEndpoints.class);
    }
}
