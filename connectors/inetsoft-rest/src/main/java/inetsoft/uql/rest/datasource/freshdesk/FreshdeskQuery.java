...
public class FreshdeskQuery extends EndpointJsonQuery<FreshdeskEndpoint> {
   public FreshdeskQuery() {
      super(FreshdeskDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(FreshdeskEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, FreshdeskEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(FreshdeskEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, FreshdeskEndpoint> endpoints =
         Endpoints.load(FreshdeskEndpoints.class);
   }
}
