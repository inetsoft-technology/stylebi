...
public class GoogleCalendarQuery extends EndpointJsonQuery<GoogleCalendarEndpoint> {
   public GoogleCalendarQuery() {
      super(GoogleCalendarDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(GoogleCalendarEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, GoogleCalendarEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(GoogleCalendarEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, GoogleCalendarEndpoint> endpoints =
         Endpoints.load(GoogleCalendarEndpoints.class);
    }
}
