...
public class SalesforceReportsQuery extends EndpointJsonQuery<SalesforceReportsEndpoint> {
   public SalesforceReportsQuery() {
      super(SalesforceReportsDataSource.TYPE);
      setJsonPath("$");
   }
   ...
   public String getSuffix() {
      ...
      SalesforceReportsDataSource dataSource = (SalesforceReportsDataSource) getDataSource();
      return suffix.replace("@apiVersion@", dataSource.getApiVersion());
   }

   @Override
   protected void updatePagination(SalesforceReportsEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.LINK_ITERATION)
            .linkParam(PaginationParamType.JSON_PATH, "$.nextPageurl")
            .build();
      }
      else {
         paginationSpec = PaginationSpec.builder()
   ...
   @Override
   public Map<String, SalesforceReportsEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(SalesforceReportsEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, SalesforceReportsEndpoint> endpoints =
         Endpoints.load(SalesforceReportsEndpoints.class);
    }
}
