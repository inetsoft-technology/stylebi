...
public class SquareQuery extends EndpointJsonQuery<SquareEndpoint> {
   public SquareQuery() {
      super(SquareDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(SquareEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.ITERATION)
            .hasNextParam(PaginationParamType.JSON_PATH, "$.cursor")
            .pageOffsetParamToRead(PaginationParamType.JSON_PATH, "$.cursor")
   ...
   @Override
   public Map<String, SquareEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(SquareEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, SquareEndpoint> endpoints =
         Endpoints.load(SquareEndpoints.class);
    }
}
