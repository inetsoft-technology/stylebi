...
public class SmartsheetQuery extends EndpointJsonQuery<SmartsheetEndpoint> {
   public SmartsheetQuery() {
      super(SmartsheetDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(SmartsheetEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.PAGE_COUNT)
            .totalPagesParam(PaginationParamType.JSON_PATH, "$.totalPages")
   ...
   @Override
   public Map<String, SmartsheetEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(SmartsheetEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, SmartsheetEndpoint> endpoints =
         Endpoints.load(SmartsheetEndpoints.class);
   }
}
