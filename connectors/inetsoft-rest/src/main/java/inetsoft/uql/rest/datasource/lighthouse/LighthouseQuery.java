...
public class LighthouseQuery extends EndpointJsonQuery<LighthouseEndpoint> {
   public LighthouseQuery() {
      super(LighthouseDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(LighthouseEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            ...
            .recordCountPath("$.tickets.length()")
            .build();
      }
      else {
         paginationSpec = PaginationSpec.builder()
   ...
   @Override
   public Map<String, LighthouseEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(LighthouseEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, LighthouseEndpoint> endpoints =
         Endpoints.load(LighthouseEndpoints.class);
    }
}
