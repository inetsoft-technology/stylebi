...
public class ChargifyQuery extends EndpointJsonQuery<ChargifyEndpoint> {
   public ChargifyQuery() {
      super(ChargifyDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(ChargifyEndpoint endpoint) {
      if(endpoint.isPaged()) {
         if("Invoices".equals(endpoint.getName())) {
            paginationSpec = PaginationSpec.builder()
               .type(PaginationType.TOTAL_COUNT_AND_PAGE)
               .totalCountParam(PaginationParamType.JSON_PATH, "$.meta.total_invoice_count")
         ...
         else if("Invoice Events".equals(endpoint.getName())) {
            paginationSpec = PaginationSpec.builder()
               .type(PaginationType.PAGE_COUNT)
               .totalPagesParam(PaginationParamType.JSON_PATH, "$.total_pages")
               .pageNumberParamToWrite(PaginationParamType.QUERY, "page")
               .maxResultsPerPageParam(PaginationParamType.QUERY, "per_page")
               .maxResultsPerPage(200)
               .build();
   ...
   @Override
   public Map<String, ChargifyEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(ChargifyEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, ChargifyEndpoint> endpoints =
         Endpoints.load(ChargifyEndpoints.class);
   }
}
