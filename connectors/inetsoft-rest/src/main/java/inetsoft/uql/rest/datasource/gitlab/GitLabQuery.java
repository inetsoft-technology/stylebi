...
public class GitLabQuery extends EndpointJsonQuery<GitLabEndpoint> {
   public GitLabQuery() {
      super(GitLabDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(GitLabEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, GitLabEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return EndpointJsonQuery.Endpoints.load(GitLabEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, GitLabEndpoint> endpoints =
         EndpointJsonQuery.Endpoints.load(GitLabEndpoints.class);
   }
}
