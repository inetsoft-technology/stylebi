...
public class TeamDeskQuery extends EndpointJsonQuery<TeamDeskEndpoint> {
   public TeamDeskQuery() {
      super(TeamDeskDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(TeamDeskEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, TeamDeskEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(TeamDeskEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, TeamDeskEndpoint> endpoints =
         Endpoints.load(TeamDeskEndpoints.class);
    }
}
