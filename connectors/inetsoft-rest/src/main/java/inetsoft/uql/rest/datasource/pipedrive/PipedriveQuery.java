...
public class PipedriveQuery extends EndpointJsonQuery<PipedriveEndpoint> {
   public PipedriveQuery() {
      super(PipedriveDataSource.TYPE);
      setJsonPath("$");
   }
   ...
   @Override
   public Map<String, PipedriveEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(PipedriveEndpoints.class);
      }
