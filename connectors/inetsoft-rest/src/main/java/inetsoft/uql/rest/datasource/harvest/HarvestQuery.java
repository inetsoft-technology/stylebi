...
public class HarvestQuery extends EndpointJsonQuery<HarvestEndpoint> {
   public HarvestQuery() {
      super(HarvestDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(HarvestEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, HarvestEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(HarvestEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, HarvestEndpoint> endpoints = Endpoints.load(HarvestEndpoints.class);
   }
}
