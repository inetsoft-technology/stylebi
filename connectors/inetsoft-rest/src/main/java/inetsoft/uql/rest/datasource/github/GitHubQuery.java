...
public class GitHubQuery extends EndpointJsonQuery<GitHubEndpoint> {
   public GitHubQuery() {
      super(GitHubDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(GitHubEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, GitHubEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(GitHubEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, GitHubEndpoint> endpoints = Endpoints.load(GitHubEndpoints.class);
   }
}
