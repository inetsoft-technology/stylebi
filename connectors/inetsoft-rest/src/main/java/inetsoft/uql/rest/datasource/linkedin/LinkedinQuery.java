...
public class LinkedinQuery extends EndpointJsonQuery<LinkedinEndpoint> {
   public LinkedinQuery() {
      super(LinkedinDataSource.TYPE);
      setJsonPath("$");
   }
   ...
   @Override
   public Map<String, LinkedinEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(LinkedinEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, LinkedinEndpoint> endpoints =
         Endpoints.load(LinkedinEndpoints.class);
    }
}
