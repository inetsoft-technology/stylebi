...
public class JiveQuery extends EndpointJsonQuery<JiveEndpoint> {
   public JiveQuery() {
      super(JiveDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(JiveEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.LINK_ITERATION)
            .linkParam(PaginationParamType.JSON_PATH, "$.links.next")
            .build();
      }
      else {
         paginationSpec = PaginationSpec.builder()
   ...
   @Override
   public Map<String, JiveEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(JiveEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, JiveEndpoint> endpoints =
         Endpoints.load(JiveEndpoints.class);
    }
}
