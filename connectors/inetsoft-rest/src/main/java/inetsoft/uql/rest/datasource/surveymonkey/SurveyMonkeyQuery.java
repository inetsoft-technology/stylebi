...
public class SurveyMonkeyQuery extends EndpointJsonQuery<SurveyMonkeyEndpoint> {
   public SurveyMonkeyQuery() {
      super(SurveyMonkeyDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(SurveyMonkeyEndpoint endpoint) {
      if(endpoint.isPaged()) {
         final int maxRows;

         if(endpoint.getPageLimit() > 0) {
         ...
         final PaginationSpec.Builder builder = PaginationSpec.builder()
            ...
            .maxResultsPerPage(maxRows)
            .recordCountPath("$.data.length()")
            .pageNumberParamToWrite(PaginationParamType.QUERY, "page");

         if(maxRows > 0) {
            builder.maxResultsPerPageParam(PaginationParamType.QUERY, "per_page");
         }

         paginationSpec = builder.build();
      }
      else {
   ...
   @Override
   public Map<String, SurveyMonkeyEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(SurveyMonkeyEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, SurveyMonkeyEndpoint> endpoints =
         Endpoints.load(SurveyMonkeyEndpoints.class);
   }
}
