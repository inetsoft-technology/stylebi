...
public class CopperQuery extends EndpointJsonQuery<CopperEndpoint> {
   public CopperQuery() {
      super(CopperDataSource.TYPE);
      setJsonPath("$");
   }
   ...
   @Override
   protected void updatePagination(CopperEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec.setMaxResultsPerPage(200);
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.TOTAL_COUNT_AND_PAGE)
            .totalCountParam(PaginationParamType.HEADER, "X-PW-TOTAL")
            .pageNumberParamToWrite(PaginationParamType.QUERY, "page_number")
            .maxResultsPerPageParam(PaginationParamType.QUERY, "page_size")
   ...
   @Override
   public Map<String, CopperEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(CopperEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, CopperEndpoint> endpoints = Endpoints.load(CopperEndpoints.class);
   }
