...
public class BoxQuery extends EndpointJsonQuery<BoxEndpoint> {
   public BoxQuery() {
      super(BoxDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(BoxEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, BoxEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(BoxEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, BoxEndpoint> endpoints =
         Endpoints.load(BoxEndpoints.class);
    }
}
