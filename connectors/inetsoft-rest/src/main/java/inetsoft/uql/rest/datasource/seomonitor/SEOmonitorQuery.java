...
public class SEOmonitorQuery extends EndpointJsonQuery<SEOmonitorEndpoint> {
   public SEOmonitorQuery() {
      super(SEOmonitorDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(SEOmonitorEndpoint endpoint) {
      if(endpoint.getPageType() == 1) {
         paginationSpec = PaginationSpec.builder()
            ...
            .totalCountParam(PaginationParamType.JSON_PATH, "$.total_results")
   ...
   @Override
   public Map<String, SEOmonitorEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(SEOmonitorEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, SEOmonitorEndpoint> endpoints =
         Endpoints.load(SEOmonitorEndpoints.class);
    }
}
