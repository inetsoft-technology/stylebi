...
public class FreshsalesQuery extends EndpointJsonQuery<FreshsalesEndpoint> {
   public FreshsalesQuery() {
      super(FreshsalesDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(FreshsalesEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, FreshsalesEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return EndpointJsonQuery.Endpoints.load(FreshsalesEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, FreshsalesEndpoint> endpoints =
         EndpointJsonQuery.Endpoints.load(FreshsalesEndpoints.class);
   }
}
