...
public class ChartMogulQuery extends EndpointJsonQuery<ChartMogulEndpoint> {
   public ChartMogulQuery() {
      super(ChartMogulDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(ChartMogulEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, ChartMogulEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return EndpointJsonQuery.Endpoints.load(ChartMogulEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, ChartMogulEndpoint> endpoints =
         EndpointJsonQuery.Endpoints.load(ChartMogulEndpoints.class);
   }
}
