...
public class StripeQuery extends EndpointJsonQuery<StripeEndpoint> {
   public StripeQuery() {
      super(StripeDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(StripeEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, StripeEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(StripeEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, StripeEndpoint> endpoints =
         Endpoints.load(StripeEndpoints.class);
    }
}
