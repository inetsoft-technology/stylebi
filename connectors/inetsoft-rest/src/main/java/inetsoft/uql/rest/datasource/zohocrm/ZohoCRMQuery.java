...
public class ZohoCRMQuery extends EndpointJsonQuery<ZohoCRMEndpoint> {
   public ZohoCRMQuery() {
      super(ZohoCRMDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(ZohoCRMEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.ITERATION)
            .hasNextParam(PaginationParamType.JSON_PATH, "$.info.more_records")
            .pageOffsetParamToRead(PaginationParamType.JSON_PATH, "$.info.page")
   ...
   @Override
   public Map<String, ZohoCRMEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(ZohoCRMEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, ZohoCRMEndpoint> endpoints =
         Endpoints.load(ZohoCRMEndpoints.class);
    }
}
