...
public class AsanaQuery extends EndpointJsonQuery<AsanaEndpoint> {
   public AsanaQuery() {
      super(AsanaDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(AsanaEndpoint endpoint) {
      if(endpoint.isPaged()) {
         // some endpoint requires a parameter to be specified for paging
         // e.g. Users requires Workspace
         String required = endpoint.getPageRequiredParameter();

         if(required == null || getParameters().findParameter(required).getValue() != null) {
            paginationSpec = PaginationSpec.builder()
               .type(PaginationType.ITERATION)
               .hasNextParam(PaginationParamType.JSON_PATH, "$.next_page")
               .pageOffsetParamToRead(PaginationParamType.JSON_PATH, "$.next_page.offset")
   ...
   @Override
   public Map<String, AsanaEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(AsanaEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, AsanaEndpoint> endpoints =
         Endpoints.load(AsanaEndpoints.class);
    }
}
