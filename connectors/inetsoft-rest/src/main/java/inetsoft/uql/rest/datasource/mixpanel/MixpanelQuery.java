...
public class MixpanelQuery extends EndpointJsonQuery<MixpanelEndpoint> {
   public MixpanelQuery() {
      super(MixpanelDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(MixpanelEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, MixpanelEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(MixpanelEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, MixpanelEndpoint> endpoints =
         Endpoints.load(MixpanelEndpoints.class);
    }
}
