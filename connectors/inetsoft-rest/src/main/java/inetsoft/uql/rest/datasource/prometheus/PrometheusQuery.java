...
public class PrometheusQuery extends EndpointJsonQuery<PrometheusEndpoint> {
   public PrometheusQuery() {
      super(PrometheusDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   public Map<String, PrometheusEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(PrometheusEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, PrometheusEndpoint> endpoints =
         Endpoints.load(PrometheusEndpoints.class);
    }
}
