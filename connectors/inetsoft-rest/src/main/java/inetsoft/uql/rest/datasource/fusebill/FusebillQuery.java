...
public class FusebillQuery extends EndpointJsonQuery<FusebillEndpoint> {
   public FusebillQuery() {
      super(FusebillDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(FusebillEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.PAGE_COUNT)
            .pageNumberParamToWrite(PaginationParamType.QUERY, "pageNumber")
            .maxResultsPerPageParam(PaginationParamType.QUERY, "pageSize")
            .totalPagesParam(PaginationParamType.HEADER, "X-MaxPageIndex")
   ...
   @Override
   public Map<String, FusebillEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(FusebillEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, FusebillEndpoint> endpoints =
         Endpoints.load(FusebillEndpoints.class);
   }
}
