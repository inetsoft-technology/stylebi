...
public class ChargebeeQuery extends EndpointJsonQuery<ChargebeeEndpoint> {
   public ChargebeeQuery() {
      super(ChargebeeDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(ChargebeeEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            ...
            .hasNextParam(PaginationParamType.JSON_PATH, "$.next_offset")
            .pageOffsetParamToRead(PaginationParamType.JSON_PATH, "$.next_offset")
   ...
   @Override
   public Map<String, ChargebeeEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return EndpointJsonQuery.Endpoints.load(ChargebeeEndpoints.class);
      }

      return ChargebeeQuery.Singleton.INSTANCE.endpoints;
   }

   enum Singleton {
      INSTANCE;
      private final Map<String, ChargebeeEndpoint> endpoints =
         EndpointJsonQuery.Endpoints.load(ChargebeeEndpoints.class);
   }
}
