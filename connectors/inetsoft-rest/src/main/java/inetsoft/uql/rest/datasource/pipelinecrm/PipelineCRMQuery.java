...
public class PipelineCRMQuery extends EndpointJsonQuery<PipelineCRMEndpoint> {
   public PipelineCRMQuery() {
      super(PipelineCRMDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(PipelineCRMEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            ...
            .totalCountParam(PaginationParamType.JSON_PATH, "$.pagination.total")
   ...
   @Override
   public Map<String, PipelineCRMEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(PipelineCRMEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, PipelineCRMEndpoint> endpoints =
         Endpoints.load(PipelineCRMEndpoints.class);
    }
}
