...
/**
 * Passes quickbooks data source type to the tabular query so the QuickbookRuntime class is used
 * instead of the SparkSQLRuntime to execute our query
 */
...
public class QuickbooksReportsQuery extends EndpointJsonQuery<QuickbooksReportsEndpoint> {
   public QuickbooksReportsQuery() {
      super(QuickbooksReportsDataSource.TYPE);
   }
   ...
   @Override
   public Map<String, QuickbooksReportsEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(QuickbooksReportsEndpoints.class);
      }

      return QuickbooksReportsQuery.Singleton.INSTANCE.endpoints;
   }

   enum Singleton {
      INSTANCE;
      private final Map<String, QuickbooksReportsEndpoint> endpoints =
         Endpoints.load(QuickbooksReportsEndpoints.class);
   }
}
