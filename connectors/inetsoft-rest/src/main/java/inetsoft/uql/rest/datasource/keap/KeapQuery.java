...
public class KeapQuery extends EndpointJsonQuery<KeapEndpoint> {
   public KeapQuery() {
      super(KeapDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(KeapEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.TOTAL_COUNT_AND_OFFSET)
            .totalCountParam(PaginationParamType.JSON_PATH, "$.count")
   ...
   @Override
   public Map<String, KeapEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(KeapEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, KeapEndpoint> endpoints =
         Endpoints.load(KeapEndpoints.class);
   }
}
