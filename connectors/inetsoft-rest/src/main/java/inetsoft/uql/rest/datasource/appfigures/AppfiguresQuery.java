...
public class AppfiguresQuery extends EndpointJsonQuery<AppfiguresEndpoint> {
   public AppfiguresQuery() {
      super(AppfiguresDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   public void setJsonPath(String jsonpath) {
      if(Objects.equals(jsonpath, super.getJsonPath()) && endpointChanged) {
         if("Categories".equals(getEndpoint())) {
            super.setJsonPath("$.*");
         }
         else {
            super.setJsonPath("$");
   ...
   public void setEndpoint(String endpoint) {
      endpointChanged = !Objects.equals(endpoint, getEndpoint());
      super.setEndpoint(endpoint);
   }

   @Override
   protected void updatePagination(AppfiguresEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, AppfiguresEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(AppfiguresEndpoints.class);
      }

      return Singleton.INSTANCE.endpoints;
   }

   private boolean endpointChanged = false;

   enum Singleton {
      INSTANCE;
      private final Map<String, AppfiguresEndpoint> endpoints =
         Endpoints.load(AppfiguresEndpoints.class);
    }
}
