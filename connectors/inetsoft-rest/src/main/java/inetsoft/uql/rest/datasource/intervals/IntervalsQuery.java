...
public class IntervalsQuery extends EndpointJsonQuery<IntervalsEndpoint> {
   public IntervalsQuery() {
      super(IntervalsDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(IntervalsEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.TOTAL_COUNT_AND_OFFSET)
            .totalCountParam(PaginationParamType.JSON_PATH, "$.listcount")
   ...
   @Override
   public Map<String, IntervalsEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(IntervalsEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, IntervalsEndpoint> endpoints =
         Endpoints.load(IntervalsEndpoints.class);
   }
}
