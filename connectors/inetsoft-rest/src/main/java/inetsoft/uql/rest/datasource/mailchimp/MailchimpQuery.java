...
public class MailchimpQuery extends EndpointJsonQuery<MailchimpEndpoint>  {
   public MailchimpQuery() {
      super(MailchimpDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(MailchimpEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.TOTAL_COUNT_AND_OFFSET)
            .totalCountParam(PaginationParamType.JSON_PATH, "total_items")
   ...
   @Override
   public Map<String, MailchimpEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(MailchimpEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, MailchimpEndpoint> endpoints =
         Endpoints.load(MailchimpEndpoints.class);
   }
