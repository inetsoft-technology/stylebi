...
public class ServiceNowQuery extends EndpointJsonQuery<ServiceNowEndpoint> {
   public ServiceNowQuery() {
      super(ServiceNowDataSource.TYPE);
      setJsonPath("$.result");
   }
   ...
   @Override
   public Map<String, ServiceNowEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(ServiceNowEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, ServiceNowEndpoint> endpoints =
         Endpoints.load(ServiceNowEndpoints.class);
   }
}
