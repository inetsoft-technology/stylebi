...
public class CampaignMonitorQuery extends EndpointJsonQuery<CampaignMonitorEndpoint> {
   public CampaignMonitorQuery() {
      super(CampaignMonitorDataSource.TYPE);
   }

   @Override
   protected void updatePagination(CampaignMonitorEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.PAGE_COUNT)
            .totalPagesParam(PaginationParamType.JSON_PATH, "$.NumberOfPages")
   ...
   @Override
   public Map<String, CampaignMonitorEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(CampaignMonitorEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, CampaignMonitorEndpoint> endpoints =
         Endpoints.load(CampaignMonitorEndpoints.class);
   }
}
