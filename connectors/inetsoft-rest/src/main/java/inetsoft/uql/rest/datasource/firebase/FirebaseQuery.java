...
public class FirebaseQuery extends EndpointJsonQuery<FirebaseEndpoint> {
   public FirebaseQuery() {
      super(FirebaseDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(FirebaseEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, FirebaseEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(FirebaseEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, FirebaseEndpoint> endpoints =
         Endpoints.load(FirebaseEndpoints.class);
    }
}
