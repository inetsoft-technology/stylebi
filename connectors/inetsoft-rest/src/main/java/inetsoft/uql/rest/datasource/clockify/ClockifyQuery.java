...
public class ClockifyQuery extends EndpointJsonQuery<ClockifyEndpoint> {
   public ClockifyQuery() {
      super(ClockifyDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   public String getURL() {
      ClockifyEndpoint endpoint = getEndpointMap().get(getEndpoint());
   ...
   @Override
   protected void updatePagination(ClockifyEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.PAGE)
            .pageNumberParamToWrite(PaginationParamType.QUERY, "page")
            .maxResultsPerPageParam(PaginationParamType.QUERY, "page-size")
            .maxResultsPerPage(200)
            .recordCountPath("$.length()")
            .build();
      }
      else if(endpoint.getName().equals("Detailed Report")) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.PAGE)
            .pageNumberParamToWrite(PaginationParamType.JSON_PATH, "$.detailedFilter.page")
            .maxResultsPerPageParam(PaginationParamType.JSON_PATH, "$.detailedFilter.pageSize")
            .maxResultsPerPage(200)
            .zeroBasedPageIndex(false)
            .recordCountPath("$.timeentries.length()")
            .build();
      }
      else {
         paginationSpec = PaginationSpec.builder()
   ...
   @Override
   public Map<String, ClockifyEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(ClockifyEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, ClockifyEndpoint> endpoints =
         Endpoints.load(ClockifyEndpoints.class);
   }
