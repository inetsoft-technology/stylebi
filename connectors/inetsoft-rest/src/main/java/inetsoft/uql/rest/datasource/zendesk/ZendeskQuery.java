...
public class ZendeskQuery extends EndpointJsonQuery<ZendeskEndpoint> {
   public ZendeskQuery() {
      super(ZendeskDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(ZendeskEndpoint endpoint) {
      switch(endpoint.getPageType()) {
      case "link":
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.LINK_ITERATION)
            .linkParam(PaginationParamType.JSON_PATH, "$.next_page")
            .build();
         break;
      case "cursor":paginationSpec = PaginationSpec.builder()
         .type(PaginationType.LINK_ITERATION)
         .linkParam(PaginationParamType.JSON_PATH, "$.after_url")
         .build();
         break;
      default:
         paginationSpec = PaginationSpec.builder()
   ...
   @Override
   public Map<String, ZendeskEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(ZendeskEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, ZendeskEndpoint> endpoints =
         Endpoints.load(ZendeskEndpoints.class);
    }
}
