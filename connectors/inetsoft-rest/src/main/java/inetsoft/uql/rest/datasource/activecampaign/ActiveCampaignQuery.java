...
public class ActiveCampaignQuery extends EndpointJsonQuery<ActiveCampaignEndpoint> {
   public ActiveCampaignQuery() {
      super(ActiveCampaignDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(ActiveCampaignEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.TOTAL_COUNT_AND_OFFSET)
            .totalCountParam(PaginationParamType.JSON_PATH, "$.meta.total")
   ...
   @Override
   public Map<String, ActiveCampaignEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return EndpointJsonQuery.Endpoints.load(ActiveCampaignEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, ActiveCampaignEndpoint> endpoints =
         EndpointJsonQuery.Endpoints.load(ActiveCampaignEndpoints.class);
   }
}
