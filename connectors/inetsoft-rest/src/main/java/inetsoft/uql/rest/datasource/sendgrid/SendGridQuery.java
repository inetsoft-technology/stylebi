...
public class SendGridQuery extends EndpointJsonQuery<SendGridEndpoint> {
   public SendGridQuery() {
      super(SendGridDataSource.TYPE);
      setJsonPath("$");
   }
   ...
   @Override
   public Map<String, SendGridEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(SendGridEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, SendGridEndpoint> endpoints =
         Endpoints.load(SendGridEndpoints.class);
    }
}
