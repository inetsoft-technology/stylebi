...
public class ZendeskSellQuery extends EndpointJsonQuery<ZendeskSellEndpoint> {
   public ZendeskSellQuery() {
      super(ZendeskSellDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(ZendeskSellEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.LINK_ITERATION)
            .linkParam(PaginationParamType.JSON_PATH, "$.meta.links.next_page")
            .build();
      }
      else {
         paginationSpec = PaginationSpec.builder()
   ...
   @Override
   public Map<String, ZendeskSellEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(ZendeskSellEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, ZendeskSellEndpoint> endpoints =
         Endpoints.load(ZendeskSellEndpoints.class);
    }
}
