...
public class DataDotWorldQuery extends EndpointJsonQuery<DataDotWorldEndpoint> {
   public DataDotWorldQuery() {
      super(DataDotWorldDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(DataDotWorldEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            ...
            .pageOffsetParamToWrite(PaginationParamType.QUERY, "next")
   ...
   @Override
   public Map<String, DataDotWorldEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(DataDotWorldEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, DataDotWorldEndpoint> endpoints =
         Endpoints.load(DataDotWorldEndpoints.class);
    }
}
