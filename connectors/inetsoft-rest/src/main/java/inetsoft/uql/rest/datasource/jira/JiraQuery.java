...
public class JiraQuery extends EndpointJsonQuery<JiraEndpoint> {
   public JiraQuery() {
      super(JiraDataSource.TYPE);
      setJsonPath("$");
   }
   ...
   @Override
   public Map<String, JiraEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(JiraEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, JiraEndpoint> endpoints =
         Endpoints.load(JiraEndpoints.class);
    }
}
