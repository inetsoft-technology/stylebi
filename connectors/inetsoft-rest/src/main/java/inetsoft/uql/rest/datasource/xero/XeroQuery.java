...
public class XeroQuery extends EndpointJsonQuery<XeroEndpoint> {
   public XeroQuery() {
      super(XeroDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(XeroEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.PAGE)
            .pageNumberParamToWrite(PaginationParamType.QUERY, "page")
            .recordCountPath(endpoint.getPaginationPath())
            .build();
      }
      else {
         paginationSpec = PaginationSpec.builder()
   ...
   @Override
   public Map<String, XeroEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(XeroEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, XeroEndpoint> endpoints =
         Endpoints.load(XeroEndpoints.class);
    }
}
