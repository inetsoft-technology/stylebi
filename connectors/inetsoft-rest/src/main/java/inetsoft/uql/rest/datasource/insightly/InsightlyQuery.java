...
public class InsightlyQuery extends EndpointJsonQuery<InsightlyEndpoint> {
   public InsightlyQuery() {
      super(InsightlyDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(InsightlyEndpoint endpoint) {
      if(endpoint.isPaged()) {
         paginationSpec = PaginationSpec.builder()
            .type(PaginationType.TOTAL_COUNT_AND_OFFSET)
            .maxResultsPerPageParam(PaginationParamType.QUERY, "top")
   ...
   @Override
   public Map<String, InsightlyEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(InsightlyEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, InsightlyEndpoint> endpoints =
         Endpoints.load(InsightlyEndpoints.class);
    }
}
