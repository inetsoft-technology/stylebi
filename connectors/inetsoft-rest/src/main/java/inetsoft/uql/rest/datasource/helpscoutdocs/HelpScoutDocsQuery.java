...
public class HelpScoutDocsQuery extends EndpointJsonQuery<HelpScoutDocsEndpoint> {
   public HelpScoutDocsQuery() {
      super(HelpScoutDocsDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(HelpScoutDocsEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, HelpScoutDocsEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(HelpScoutDocsEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, HelpScoutDocsEndpoint> endpoints =
         Endpoints.load(HelpScoutDocsEndpoints.class);
    }
}
