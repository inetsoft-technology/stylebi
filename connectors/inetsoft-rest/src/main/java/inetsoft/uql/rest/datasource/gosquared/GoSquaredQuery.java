...
public class GoSquaredQuery extends EndpointJsonQuery<GoSquaredEndpoint> {
   public GoSquaredQuery() {
      super(GoSquaredDataSource.TYPE);
      setJsonPath("$");
      paginationSpec = PaginationSpec.builder()
         .type(PaginationType.LINK_ITERATION)
         .linkParam(PaginationParamType.JSON_PATH, "$._links[?(@.rel == 'next')].href")
         .build();
   }

   @Override
   protected void updatePagination(GoSquaredEndpoint endpoint) {
      // no-op
   }

   @Override
   public Map<String, GoSquaredEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(GoSquaredEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, GoSquaredEndpoint> endpoints =
         Endpoints.load(GoSquaredEndpoints.class);
   }
}
