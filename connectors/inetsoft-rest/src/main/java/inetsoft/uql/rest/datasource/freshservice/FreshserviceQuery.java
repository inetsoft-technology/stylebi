...
public class FreshserviceQuery extends EndpointJsonQuery<FreshserviceEndpoint> {
   public FreshserviceQuery() {
      super(FreshserviceDataSource.TYPE);
      setJsonPath("$");
   }
   ...
   @Override
   public Map<String, FreshserviceEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(FreshserviceEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, FreshserviceEndpoint> endpoints =
         Endpoints.load(FreshserviceEndpoints.class);
    }
}
