...
public class NicereplyQuery extends EndpointJsonQuery<NicereplyEndpoint> {
   public NicereplyQuery() {
      super(NicereplyDataSource.TYPE);
      setJsonPath("$");
   }

   @Override
   protected void updatePagination(NicereplyEndpoint endpoint) {
      if(endpoint.isPaged()) {
   ...
   @Override
   public Map<String, NicereplyEndpoint> getEndpointMap() {
      if("true".equals(SreeEnv.getProperty("debug.endpoints"))) {
         return Endpoints.load(NicereplyEndpoints.class);
      }
   ...
   enum Singleton {
      INSTANCE;
      private final Map<String, NicereplyEndpoint> endpoints =
         Endpoints.load(NicereplyEndpoints.class);
   }
}
