...
public class ODataHttpClientFactory extends DefaultHttpClientFactory {
   public ODataHttpClientFactory(ODataDataSource dataSource, boolean saveTokens) {
   ...
   public DefaultHttpClient create(HttpMethod method, URI uri) {
      ...
      client.addRequestInterceptor((request, context) -> {
         Tokens tokens = AuthorizationClient.refreshTokens(dataSource, false);
   ...
   private static final Logger LOG = LoggerFactory.getLogger(ODataHttpClientFactory.class);
}
