...
public class mxGeometry extends mxRectangle {
   ...
   // outer width and outer height are used to check for overlapping only, which may
   // include text labels outside of a node bounds.
   private double outerWidth, outerHeight;
   ...
   public void transformPosition(AffineTransform trans) {
      super.transformPosition(trans);

      if(points != null) {
         points.forEach(p -> p.transformPosition(trans));
      }
   }
   ...
   public void transform(AffineTransform trans) {
      ...
      if(alternateBounds != null) {
         alternateBounds.transform(trans);
      }

      if(sourcePoint != null) {
         sourcePoint.transform(trans);
      }

      if(targetPoint != null) {
         targetPoint.transform(trans);
      }
      ...
      if(points != null) {
         points.forEach(p -> p.transform(trans));
      }
   }
