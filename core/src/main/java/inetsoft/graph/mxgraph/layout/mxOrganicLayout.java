...
import inetsoft.graph.mxgraph.view.mxGraphView;
...
public class mxOrganicLayout extends mxGraphLayout {
   ...
   public void execute(Object parent)
   {
      ...
      for(int i = 0; i < vertices.length; i++) {
         ...
         vertexMap.put(vertices[i], Integer.valueOf(i));
   ...
   protected double getNodeDistribution(int i) {
      ...
      if(isOptimizeNodeDistribution) {
         ...
         for(int j = 0; j < v.length; j++) {
            if(i != j) {
               CellWrapper src = v[i];
               CellWrapper target = v[j];

               if(src.getBounds().intersects(target.getBounds())) {
                  Rectangle2D overlap = src.getBounds().createIntersection(target.getBounds());
                  double dist = Point2D.distanceSq(src.x, src.y, target.x, target.y);

                  if(dist != 0) {
                     energy += 16000 / dist;
                  }

                  // increase weight to avoid overlapping. (56755)
                  //energy += 0.1 * overlap.getWidth() * overlap.getHeight();
                  energy += 1.5 * overlap.getWidth() * overlap.getHeight();
               }
            }
         }
   ...
   protected double getEdgeLength(int i) {
      if(isOptimizeEdgeLength) {
         double edgeLength = Point2D.distance(v[e[i].source].x, v[e[i].source].y,
                                              v[e[i].target].x, v[e[i].target].y);
   ...
   protected int[] getRelevantEdges(int cellIndex) {
      ...
      for(int i = 0; i < e.length; i++) {
         if(e[i].source != cellIndex && e[i].target != cellIndex) {
            ...
            relevantEdgeList.add(Integer.valueOf(i));
         }
      }
   ...
   protected int[] getConnectedEdges(int cellIndex)
   {
      ...
      for(int i = 0; i < e.length; i++) {
         if(e[i].source == cellIndex || e[i].target == cellIndex) {
            ...
            connectedEdgeList.add(Integer.valueOf(i));
         }
      }
   ...
   public class CellWrapper {
      ...
      public Rectangle2D getBounds() {
         // use outer size for checking overlapping
         double w = ((mxCell) cell).getGeometry().getOuterWidth();
         double h = ((mxCell) cell).getGeometry().getOuterHeight();

         if(w == 0 || h == 0) {
            w = ((mxCell) cell).getGeometry().getWidth();
            h = ((mxCell) cell).getGeometry().getHeight();
         }
