...
import inetsoft.graph.mxgraph.costfunction.mxCostFunction;
import inetsoft.graph.mxgraph.costfunction.mxDoubleValCostFunction;
import inetsoft.graph.mxgraph.generatorfunction.mxGeneratorFunction;
import inetsoft.graph.mxgraph.generatorfunction.mxGeneratorRandomFunction;
...
public class mxGraphGenerator {
   ...
   public void getFriendshipWindmillGraph(mxAnalysisGraph aGraph, int numBranches, int branchSize)
   {
      ...
      for(int i = 0; i < numBranches; i++) {
         for(int j = 0; j < branchSize; j++) {
            vertices[vertexCount] = graph.insertVertex(parent, null, Integer.valueOf(vertexCount).toString(), 0, 0, 25, 25);
            vertexCount++;
         }
      }

      vertices[numVertices - 1] = graph.insertVertex(parent, null, Integer.valueOf(numVertices - 1).toString(), 0, 0, 25, 25);
   ...
   public void getKnightTour(mxAnalysisGraph aGraph, int xDim, int yDim, int startVertexValue) throws StructuralException
   {
      ...
      for(int i = 0; i < vertexNum; i++) {
         graph.insertVertex(parent, null, Integer.valueOf(vertexCount).toString(), 0, 0, 25, 25);
         vertexCount++;
      }
