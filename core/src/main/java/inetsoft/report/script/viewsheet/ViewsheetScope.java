...
public class ViewsheetScope extends ScriptableObject implements Cloneable, DynamicScope {
   ...
   /**
    * Execute a script and return a structured result instead of throwing.
    *
    * <p>This method is the agent-facing counterpart to
    * {@link #execute(String, VSAScriptable, boolean)}: it applies the same sandbox
    * constraints and principal context but, on error, returns a {@link ScriptExecResult}
    * carrying the message and Rhino suggestion rather than logging and rethrowing.
    * Existing callers of {@code execute()} are not affected.</p>
    *
    * <p>The script is compiled fresh (bypassing the shared {@link ScriptCache}) so
    * that a compilation error does not cache a {@code null} entry that would silently
    * mask the error from subsequent normal {@code execute()} calls on the same text.</p>
    *
    * <p><strong>Security:</strong> the same {@code SecureClassShutter}/sandbox and
    * {@link inetsoft.report.script.formula.FormulaContext} restrictions apply as in
    * {@code execute()}.</p>
    *
    * @param statement  the script text to compile and execute
    * @param scriptable the scope in which to execute; may be {@code null} to run in
    *                   the viewsheet scope itself
    * @return a {@link ScriptExecResult} with {@code ok=true} and the return value on
    *         success, or {@code ok=false} and a {@link inetsoft.web.wiz.script.model.ScriptError}
    *         carrying the message and Rhino suggestion on failure
    */
   public inetsoft.web.wiz.script.model.ScriptExecResult executeStructured(
      String statement, VSAScriptable scriptable)
   {
      if(statement == null || statement.isEmpty()) {
         return inetsoft.web.wiz.script.model.ScriptExecResult.success(null);
      }

      // set up principal context (mirrors execute())
      if(vtable != null && !vtable.contains("__principal__") && box.getUser() != null) {
         Principal principal = box.getUser();
         vtable.put("__principal__", principal);
         vtable.put("_USER_", XUtil.getUserName(principal));
      ...
      if(scriptable != null && oldScriptable != scriptable) {
         resetCubeScriptable(scriptable);
      }

      // Compile fresh — bypass the shared scriptCache to avoid caching a null entry
      // for a broken script, which would silently mask errors from subsequent execute() calls.
      Object script;

      try {
         script = senv.compile(statement, true);
      }
      catch(Exception ex) {
         String suggestion = senv.getSuggestion(ex, null, this);
         String msg = "Script compilation error: " + ex.getMessage();
         return inetsoft.web.wiz.script.model.ScriptExecResult.failure(
            new inetsoft.web.wiz.script.model.ScriptError(msg, suggestion));
      }

      // Execute — same restrictions as execute()
      try {
         FormulaContext.setRestricted(true);
         Scriptable scope = scriptable != null ? (Scriptable) scriptable : this;
         Object result = senv.exec(script, scope, this, box.getViewsheet());
         return inetsoft.web.wiz.script.model.ScriptExecResult.success(result);
      }
      catch(Exception ex) {
         String suggestion = senv.getSuggestion(ex, null, this);
         String msg = "Script execution error: " + ex.getMessage();
