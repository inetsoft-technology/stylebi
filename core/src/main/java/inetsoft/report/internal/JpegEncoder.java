 ...
 */
// added package to fit in to style report class structure
...
import inetsoft.util.graphics.PixelGrabber;
...
public class JpegEncoder {
   ...
   public static int[] jpegNaturalOrder = { 0, 1, 8, 16, 9, 2, 3, 10, 17, 24,
      32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40, 48, 41, 34, 27, 20, 13, 6, 7,
      14, 21, 28, 35, 42, 49, 56, 57, 50, 43, 36, 29, 22, 15, 23, 30, 37, 44,
   ...
   public void Compress() {
      ...
      try {
         ...
         LOG.warn("Failed to flush output stream after compressing", e);
      }
   }
                  ...
                  for(j = 0; j < JpegObj.HsampFactor[comp]; j++) {
                     ...
                     for(a = 0; a < 8; a++) {
                        for(b = 0; b < 8; b++) {
                           ...
                           dctArray1[a][b] = inputArray[ypos + yblockoffset +
                              a][xpos + xblockoffset + b];
                        }
                     }
   ...
   public void WriteHeaders(BufferedOutputStream out) {
      ...
      JFIF[6] = (byte) 0x49; // I
      JFIF[7] = (byte) 0x46; // F
      JFIF[8] = (byte) 0x00; // eos
      JFIF[9] = (byte) 0x01; // version
      JFIF[10] = (byte) 0x02; // version

      if(resolution > 0) {
         int dpi = resolution;

         JFIF[11] = (byte) 0x01; // unit, 1 to specify dot-per-inch
         JFIF[12] = (byte) ((dpi >> 8) & 0xFF); // x density
         JFIF[13] = (byte) (dpi & 0xFF); // x density
         JFIF[14] = (byte) ((dpi >> 8) & 0xFF); // y density
         JFIF[15] = (byte) (dpi & 0xFF); // y density
      }
      else {
         JFIF[11] = (byte) 0x00; // unit, 1 to specify
         JFIF[12] = (byte) 0x00; // x density
         JFIF[13] = (byte) 0x01; // x density
         JFIF[14] = (byte) 0x00; // y density
         JFIF[15] = (byte) 0x01; // y density
      }
   ...
   void WriteMarker(byte[] data, BufferedOutputStream out) {
      try {
         ...
         LOG.warn("Failed to write marker", e);
      }
   }
   ...
   void WriteArray(byte[] data, BufferedOutputStream out) {
      ...
      try {
         ...
         LOG.warn("Failed to write array", e);
      }
   }
   ...
   /*public static void main(String args[]) {
      try {
         Image img = Tool.createImage(300, 300, false);
         FileOutputStream output = new FileOutputStream(new File("jpg.jpg"));
         ...
         /*
          ...
          output.close();
          */
         /* inetsoft.report.internal.Common.writeJPEG(img, output);
         long end2 = System.currentTimeMillis();
         LOG.debug("compress: " + (end2 - now));
      }
      catch(Exception e) {
   ...
   static final Logger LOG = LoggerFactory.getLogger(JpegEncoder.class);
}
      ...
      for(i = 0; i < 8; i++) {
         for(j = 0; j < 8; j++) {
            ...
            DivisorsChrominance[index] = (double) ((double) 1.0 /
               ((double) quantum_chrominance[index] * AANscaleFactor[i] *
               AANscaleFactor[j] * (double) 8.0));
            index++;
         }
      }
   ...
   void bufferIt(BufferedOutputStream outStream, int code, int size) {
      ...
      while(PutBits >= 8) {
         ...
         try {
            ...
            JpegEncoder.LOG.error("Failed to write data to image stream while buffering", e);
         }
   ...
   void flushBuffer(BufferedOutputStream outStream) {
      ...
      while(PutBits >= 8) {
         ...
         try {
            ...
            JpegEncoder.LOG.error("Failed to write data to image stream while flushing buffer", e);
         }
   ...
   private void getYCCArray() {
      ...
      for(y = 0; y < imageHeight; ++y) {
         for(x = 0; x < imageWidth; ++x) {
            ...
            // transparent color default to white
            if((values[index] >>> 24) == 0) {
               r = 255;
               g = 255;
               b = 255;
            }
