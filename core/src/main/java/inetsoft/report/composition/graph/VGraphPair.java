...
public class VGraphPair {
    ...
    * Note: unlike getBottomXGraphic, this path does not apply a 1px translateY correction.
    * The SVG flip transform that pushes the axis line out of the viewport is specific to the
    * SVG rendering pipeline; BufferedImage rendering does not share that transform, so no
   ...
   /**
    * Get the image for plot.
    * @param animate when false, no entrance-animation is applied to the SVG. Hover-dim CSS is
    * still injected so hover highlighting works. Used to suppress animation in design-time
    * surfaces (composer canvas, binding editor, wizard editing).
    */
   public Graphics2D getPlotGraphic(int row, int col, boolean animate) {
      ...
      // Radar charts render PolarAxis (spokes/rings) inside the plot tile, so paintAxes must
      // be true for them. All other types suppress axes to prevent label bleed into plot tiles.
      // applyHint=true: only the plot tile carries the animation/hover hint; title and axis tiles
      // have no hover annotations and must not receive it.
      return getFlipYSubGraphic(vgraph, gbounds.getPlotBounds(), row, col, true, getEVGraphContext(hasRadarCoord(vgraph), true), animate, true);
   }
   ...
   private Graphics2D getFlipYSubGraphic(VGraph graph, Rectangle2D bounds,
                                         int row, int col, boolean restrict, GraphPaintContext ctx)
   {
      // Title/axis/facet-corner tiles: no animation and no hover hint (applyHint=false).
      return getFlipYSubGraphic(graph, bounds, row, col, restrict, ctx, false, false);
   }

   private Graphics2D getFlipYSubGraphic(VGraph graph, Rectangle2D bounds,
                                         int row, int col, boolean restrict, GraphPaintContext ctx,
                                         boolean animate, boolean applyHint)
   {
                                           ...
                                           int row, int col, GraphPaintContext ctx)
   {
      ...
      // Assumption: the bottom axis is short enough that it never needs more than one
      // tile row. Row 1+ (if ever generated by unusually tall axis labels) are not shifted.
      double translateY = row == 0 ? 1D : 0D;
      // Bottom-axis tile: no animation and no hover hint (applyHint=false).
   ...
   private Graphics2D getSubGraphic(VGraph graph, Rectangle2D bounds,
                                    ...
                                    boolean animate, boolean applyHint)
   {
      return getSubGraphic(graph, bounds, row, col, restrict, ctx, animate, applyHint, 0D);
   }

   private Graphics2D getSubGraphic(VGraph graph, Rectangle2D bounds,
                                    int row, int col, boolean restrict, GraphPaintContext ctx,
                                    boolean animate, boolean applyHint, double translateY)
   {
      ...
      g.clipRect(0, 0, w, h);
      // shift by GDefaults.SVG_TILE_TRANSLATE to avoid clipping half of line. see above.
      g.translate(-subox.getX() + GDefaults.SVG_TILE_TRANSLATE,
                  -subox.getY() + GDefaults.SVG_TILE_TRANSLATE + translateY);
      graph.paintGraph(g, ctx);

      // Only the plot tile (applyHint=true) carries the animation/hover hint; title, axis and
      // facet-corner tiles have no hover annotations and must not receive it.
      ...
      // The hint is applied even when animate=false so the hover-dim CSS is injected;
      // a false animate only appends the noanim flag, which suppresses the entrance animation.
      if(applyHint) {
         applyAnimationHint(g, graph, animate);
      }
   ...
   private void applyAnimationHint(Graphics2D g, VGraph graph, boolean animate) {
      SVGSupport svgSupport = SVGSupport.getInstance();
      ...
      // Compute the base hint for the chart type; the hint is set once at the end so the noanim
      // flag can be appended uniformly. A null hint means no animation/hover is applicable
      // (e.g. 3D bar) and nothing is set.
      String hint = null;
      ...
      if(hasPieVO(graph)) {
         hint = SVGSupport.ANIMATION_PIE;
      }
      else if(hasRadarCoord(graph)) {
         hint = SVGSupport.ANIMATION_RADAR;
      }
      else if(hasSunburstVO(graph)) {
         hint = SVGSupport.ANIMATION_SUNBURST;
      }
      else if(hasIcicleVO(graph)) {
         hint = SVGSupport.ANIMATION_ICICLE;
      }
      ...
      // TreemapElement.Type values (CIRCLE vs TREEMAP), so they are mutually exclusive.
         ...
         hint = SVGSupport.ANIMATION_CIRCLE_PACKING;
      }
      else if(hasTreemapVO(graph)) {
         hint = SVGSupport.ANIMATION_TREEMAP;
      }
      else if(hasMekkoVO(graph)) {
         hint = SVGSupport.ANIMATION_MEKKO;
      }
      else if(hasRelationVO(graph)) {
         hint = SVGSupport.ANIMATION_RELATION;
      }
      else if(hasBarVO(graph)) {
         // 3D bar is deprecated — skip animation and hover entirely.
         if(!has3DBarVO(graph)) {
            hint = SVGSupport.ANIMATION_GROW;
         ...
         hint = SVGSupport.ANIMATION_LINE;
      }
      ...
      // ensures the correct animation type is selected; point-only charts reach hasPointVO.
         ...
         hint = SVGSupport.ANIMATION_CANDLE;
      }
      else if(hasBoxSchemaVO(graph)) {
         hint = SVGSupport.ANIMATION_BOX;
      }
      else if(hasPointVO(graph)) {
         hint = SVGSupport.ANIMATION_POINT;
      }

      if(hint != null) {
         // Design-time surfaces (animate=false): append the noanim flag so the injector still
         // injects hover-dim CSS but skips the entrance animation.
         if(!animate) {
            hint += ":" + SVGSupport.ANIMATION_FLAG_NOANIM;
         }
