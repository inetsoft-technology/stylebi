...
/**
 * The DoubleBorderMixed class is a table style class. It can be used with other
 * table lens classes to provide a visual style for table formatting and
 * printing. The default style for DoubleBorderMixed is:<p>
 * <img src="images/DoubleBorderMixed.gif"><p>
 ...
 */
public class DoubleBorderMixed extends TableStyle {
   /**
    * Create an empty style. The setTable() method must be called before
    * it can be used.
    */
   public DoubleBorderMixed() {
   }
    ...
    */
   public DoubleBorderMixed(TableLens table) {
      super(table);
   }
   ...
   class Style extends Transparent {
      ...
      public int getRowBorder(int r, int c) {
         return (r == -1 || r == lastRow()) ?
            DOUBLE_LINE :
            (isHeaderRowFormat(r) || isHeaderColFormat(c) ||
      ...
      public int getColBorder(int r, int c) {
         return (c == -1 || c == lastCol()) ?
            DOUBLE_LINE :
            ((!isHeaderRowFormat(r) &&
            (isHeaderColFormat(c) || isTrailerColFormat(c + 1))) ?
            THIN_LINE :
            NO_BORDER);
      }
