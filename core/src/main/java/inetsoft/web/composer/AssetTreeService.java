...
public class AssetTreeService {
    ...
    */
   public LoadAssetTreeNodesValidator getNodes(boolean includeDatasources, boolean includeColumns,
    ...
    * @return the child nodes.
    */
   private LoadAssetTreeNodesValidator getNodes0(
      ...
      throws Exception
   {
      ...
      if(!includeModel) {
         assetSelector.addExclude(AssetEntry.Type.LOGIC_MODEL);
      }
      ...
      if(expandedEntry == null) {
         ...
         if(!"cubeRoot".equals(expandedEntry.getProperty("entryName"))) {
            ...
            if(children != null && children.length > 0 && (event.publish() || event.shared())) {
               children = filterByVisualizationScope(children, event.publish(), event.shared());
            }
         ...
         if(includeColumns) {
            ...
            treeNodeModel = convertToTreeNodeModel(atmNode, catalog,
                                                   AssetTreeService::isLeafNode, sqlEnabled, principal);
         }
      }
   ...
   /**
    * Filters entries based on the visualizationScope property.
    * publish=true: keep entries where visualizationScope is null/absent or "public", plus all folders.
    * shared=true:  keep entries where visualizationScope is "shared", plus all folders.
    */
   private AssetEntry[] filterByVisualizationScope(AssetEntry[] entries,
                                                   boolean publish, boolean shared)
   {
   ...
   private void addVisualizationRootNodes(List<TreeNodeModel> children, IdentityID user,
                                          Principal principal)
   {
      AssetEntry entry = new AssetEntry(
         AssetRepository.GLOBAL_SCOPE, AssetEntry.Type.REPOSITORY_FOLDER,
                                                ...
                                                Principal user)
   {
      final Function<AssetEntry, Boolean> isLeaf =
         leafFn == null ? AssetTreeService::isLeaf : leafFn;
   ...
   public static boolean isLeafNode(AssetEntry entry) {
      return isColumnParent(entry) || entry.isViewsheet() ||
   ...
   private static TreeNodeModel createNodeFromEntry(AssetEntry entry, String label, Principal user) {
      return createNodeFromEntry(
         entry, label, Collections.emptyList(), AssetTreeService::isLeaf, user);
   }
