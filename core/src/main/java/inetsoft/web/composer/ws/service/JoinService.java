...
import inetsoft.web.composer.ws.joins.*;
...
public class JoinService {
   public void checkContainsSubtable(RuntimeWorksheet rws, String targetTable, String sourceTable) {
   ...
   public void validateWSAndDispatchCommands(
   ...
   public JoinMetaInfo joinSourceAndTargetTables(
      ...
      throws Exception
   {
      ...
      if(targetJoinTable == null) {
         ...
         if(autojoined) {
            jtable = createNewInnerJoinTable(ws, noperator);
         }
         else {
            String[] pair = new String[2];
            pair[0] = sourceTable.getAbsoluteName();
            pair[1] = targetTable.getAbsoluteName();
            jtable = CrossJoinController.process(rws, pair, true).getJoinTable();
         }
   ...
   public RelationalJoinTableAssembly createNewInnerJoinTable(
      Worksheet ws, TableAssemblyOperator noperator)
      throws Exception
   {
      ...
      for(int i = 0; i < noperator.getOperatorCount(); i++) {
         ...
         top.addOperator(op);
         joined = concatenateTable(joined, top, ws);
      }
   ...
   public static class JoinMetaInfo {
