...
public class WorksheetOpenAssetService extends WorksheetControllerService {
   ...
   public OpenAssetEventValidator checkTrap(@ClusterProxyKey String runtimeId, OpenAssetEvent event,
                         ...
                         Principal principal, CommandDispatcher commandDispatcher) throws Exception
   {
      ...
      if(entries.length == 1 && entries[0].isWorksheet()) {
         dropWorksheet(runtimeId, principal, entries, pos, commandDispatcher);
      }
      // drop worksheet columns.
      else if(entries.length > 0 &&
         VSEventUtil.BASE_WORKSHEET.equals(entries[0].getProperty("source")))
      {
         dropWorksheetColumn(runtimeId, principal, entries, pos, commandDispatcher);
      }
      // dropping table or column(s)
      else {
         dropDBData(runtimeId, principal, entries, pos, commandDispatcher);
      }
   ...
   private void dropWorksheetColumn(String runtimeId, Principal principal, AssetEntry[] entries,
                                    Point pos, CommandDispatcher commandDispatcher) throws Exception
   {
      java.util.List<AssetEntry> list = getValidEntries(entries);
      ...
      MirrorAssembly mirror = createNewMirror(runtimeId, principal, wsEntries, pos, commandDispatcher);
      hideColumns(mirror, list);
      refreshWSData(runtimeId, principal, mirror, commandDispatcher);
   }

   private void hideColumns(MirrorAssembly mirror, java.util.List<AssetEntry> list) {
   ...
   private java.util.List<AssetEntry> getValidEntries(AssetEntry[] entries) {
      AssetEntry entry = entries[0];
      String ws = entry.getProperty("ws");
      String source = entry.getProperty("source");
      java.util.List<AssetEntry> list = new ArrayList<>();
   ...
   private MirrorAssembly createNewMirror(String runtimeId, Principal principal, AssetEntry[] entries,
                                          Point pos, CommandDispatcher commandDispatcher) throws Exception
   {
      RuntimeWorksheet rws = super.getRuntimeWorksheet(runtimeId, principal);
      ...
      Worksheet src = (Worksheet) engine.getSheet(entry, rws.getUser(), true,
                                                  AssetContent.ALL);
      java.util.List errors = (java.util.List) AssetRepository.ASSET_ERRORS.get();
   ...
   private void dropWorksheet(String runtimeId, Principal principal, AssetEntry[] entries, Point pos,
                              CommandDispatcher commandDispatcher) throws Exception
   {
      MirrorAssembly mirror = createNewMirror(runtimeId, principal, entries, pos, commandDispatcher);
      refreshWSData(runtimeId, principal, mirror, commandDispatcher);
   }

   private void refreshWSData(String runtimeId, Principal principal, MirrorAssembly nassembly,
                              CommandDispatcher commandDispatcher) throws Exception
   {
      RuntimeWorksheet rws = super.getRuntimeWorksheet(runtimeId, principal);
      Worksheet ws = rws.getWorksheet();
   ...
   private void dropDBData(String runtimeId, Principal principal, AssetEntry[] entries, Point pos,
                           CommandDispatcher commandDispatcher) throws Exception
   {
      RuntimeWorksheet rws = super.getRuntimeWorksheet(runtimeId, principal);
      Worksheet ws = rws.getWorksheet();
