...
public class ImportCSVDialogService {
                                                  ...
                                                  ImportCSVDialogModel model) throws Exception
   {
      ...
      try {
         if(!csvTemp.exists()) {
            ...
            csvInfo = ImportCSVDialogService.convertModelToCSVInfo(model, TextUtil.detectType(input));
         }
      }
      finally {
   ...
   static CSVInfo convertModelToCSVInfo(ImportCSVDialogModel model, TextFileType type) {
   ...
   private void processUploadCSV(BinaryTransfer data, String rid) throws Exception {
      ...
      try(FileOutputStream fos = new FileOutputStream(temp)) {
         binaryTransferService.writeData(data, fos);
      }
   }
   ...
   private boolean isEmptyRow(Object[] rdata) {
      return rdata == null || Arrays.stream(rdata).allMatch(Objects::isNull);
   }
                                                ...
                                                XEmbeddedTable preview)
   {
      ...
      buffer.append(catalog.getString("Columns")).append(" ");
   ...
   private static final int COL_LIMIT = 1000;
   private static final Logger LOG = LoggerFactory.getLogger(ImportCSVDialogService.class);
}
