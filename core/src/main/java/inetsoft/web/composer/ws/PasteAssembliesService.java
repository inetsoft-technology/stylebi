...
public class PasteAssembliesService extends WorksheetControllerService {
   ...
   public Void pasteAssemblies(
      ...
      WSPasteAssembliesEvent event, Principal principal,
      CommandDispatcher commandDispatcher) throws Exception
   {
      OrderedMap<String, String> nameMap = new OrderedMap<>();
      ...
      String[] nnames = getSortedAvailableNames(rws, names, runtimeId, event, principal);
      ...
      for(int i = 0; i < nnames.length; i++) {
         copyAssembly(rws, nnames[i], runtimeId, event, principal,
                      nameMap, x, y, topLeft.x, topLeft.y, false);
      ...
      update(nameMap, principal, rws, tprimary, sprimary, commandDispatcher);
   ...
   private WSAssembly copyAssembly(
      RuntimeWorksheet rws, String name, String runtimeId,
      WSPasteAssembliesEvent event, Principal principal, OrderedMap<String, String> nameMap,
      int x, int y, int left, int top, boolean outer)
      throws Exception
   {
      Worksheet ws = rws.getWorksheet();
      // self worksheet entry
      Worksheet sws = getSRWS(event, principal).getWorksheet();
      AbstractWSAssembly assembly = (AbstractWSAssembly) sws.getAssembly(name);
      Point pos = assembly.getPixelOffset();
      x += pos.x - left;
      y += pos.y - top;

      String nname = getNewName(rws, name, runtimeId, event);
      nameMap.put(nname, name);
      AbstractWSAssembly nassembly = getNewAssembly(rws, assembly, nname, runtimeId, event);
      nassembly.setOuter(outer);

      if(!isSame(runtimeId, event) && isDependNamedGroup(sws, assembly) &&
    ...
    */
   private void update(
      OrderedMap<String, String> nameMap, Principal principal,
      ...
      throws Exception
   {
      ...
      e = nameMap.keys();
      java.util.List<String> pastedAssemblies = new ArrayList<>();
    ...
    */
   private String[] getSortedAvailableNames(RuntimeWorksheet rws, String[] names,
                                            String runtimeId,
                                            WSPasteAssembliesEvent event, Principal principal)
      throws Exception
   {
      Worksheet ws = getSRWS(event, principal).getWorksheet();
   ...
   private void removeUnavailableName(
      ...
      Set nameset, String[] nnames, java.util.List<String> list,
      String runtimeId, WSPasteAssembliesEvent event)
   {
      ...
      for(int i = 0; i < nnames.length; i++) {
         ...
         if(!isSame(runtimeId, event) && assembly != null &&
      ...
      checkDependent(rws, ws, nameset, escSortedNames,
                     ascSortedNames, removed, runtimeId, event);
   ...
   private void checkDependent(
      ...
      String[] ascSortedNames, java.util.List<String> removed,
      String runtimeId, WSPasteAssembliesEvent event)
   {
      if(nameset.size() == 0) {
         return;
      ...
      for(int i = 0; i < ascSortedNames.length; i++) {
         ...
         if(isCut(event) && !isSame(runtimeId, event) && assembly != null &&
   ...
   private AbstractWSAssembly getNewAssembly(
      ...
      String nname, String runtimeId, WSPasteAssembliesEvent event)
   {
      if(isCut(event) && isSame(runtimeId, event)) {
         return assembly;
      }
    ...
    */
   private boolean isCut(WSPasteAssembliesEvent event) {
      return event.getCut();
   }
    ...
    */
   private boolean isSame(String runtimeId, WSPasteAssembliesEvent event) {
      return runtimeId.equals(event.getSourceRuntimeId());
   }

   /**
    * Get source runtime worksheet.
    */
   private RuntimeWorksheet getSRWS(WSPasteAssembliesEvent event, Principal principal) throws Exception {
