...
public class AddVisualizationService {
      ...
      throws Exception
   {
      ...
      if(wizInfo != null && wizInfo.getMergedVisualizations().contains(vizEntry.toIdentifier())) {
         LOG.debug("Visualization already merged, skipping: {}", vizEntry);
         return null;
      }
                                         ...
                                         Map<String, String> vsRenameMap)
   {
      ...
      ColumnSelection      origCols  = (ColumnSelection) existingTable.getColumnSelection(true).clone();
                                ...
                                int xOffset, int yOffset, float scale)
   {
      ...
      for(Assembly a : vizVS.getAssemblies()) {
         ...
         // Resolve assembly name conflict in dashVS
         String originalName = srcAssembly.getName();
         String targetName   = originalName;
