...
public class ComposerObjectController {
    ...
    */
   public void addEmbeddedViewsheet(AssetEntry entry, RuntimeViewsheet rvs, Point position,
      ...
      throws Exception
   {
      ...
      pasteAssemblies(event, principal, dispatcher, assemblies);
   }
                            ...
                            @LinkUri String linkUri) throws Exception
   {
      ...
      resizeObject(event, principal, dispatcher, linkUri);
   }
                                     ...
                                     CommandDispatcher dispatcher) throws Exception
   {
      final RuntimeViewsheet rvs =
         this.engine.getViewsheet(this.runtimeViewsheetRef.getRuntimeId(), principal);
      ...
      if(event.objectNames().length == 1 && event.objectNames()[0] != null) {
         removeObject(event.objectNames()[0], linkUri, principal, dispatcher);
         return;
      }
   ...
   public void removeObject(@Payload RemoveVSObjectEvent event,
                            ...
                            CommandDispatcher dispatcher) throws Exception
   {
      removeObject(event.getName(), linkUri, principal, dispatcher);
   }

   public void removeObject(String objectName, String linkUri, Principal principal,
                            CommandDispatcher dispatcher) throws Exception
   {
      final RuntimeViewsheet rvs =
         this.engine.getViewsheet(this.runtimeViewsheetRef.getRuntimeId(), principal);
      ...
      throws Exception
   {
      ...
      return new DependentAssemblies(dependentAssembliesMap);
   }
         ...
         if(!(container instanceof TabVSAssembly)) {
            ...
            if(containerBounds.contains(position)) {
               ...
               if(container instanceof GroupContainerVSAssembly) {
                  moveObject(event, principal, dispatcher, linkUri);
               }
               else {
   ...
   private void pasteAssemblies(CopyVSObjectsEvent event, Principal principal,
