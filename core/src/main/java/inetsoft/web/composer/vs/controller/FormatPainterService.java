...
@Service
@ClusterProxy
public class FormatPainterService {
   public FormatPainterService(CoreLifecycleService coreLifecycleService,
   ...
   public Void getFormat(@ClusterProxyKey String runtimeId, GetVSObjectFormatEvent event,
                         Principal principal,
         ...
         || ("plot_area".equals(event.getRegion()) && event.getColumnName() != null)))
      {
         try {
            if(!event.getRegion().endsWith("_axis") || event.getColumnName() != null) {
               getChartFormat(runtimeId, event, principal, (ChartVSAssembly) assembly, commandDispatcher);
               return null;
            }
         }
      ...
      try {
         ...
         for(int i = 0; i < event.getCharts().length; i++) {
                  ...
                  (event.getColumnNames()[i] == null || event.getColumnNames()[i].length == 0)))
            {
               ...
               setChartFormat(runtimeId, event.getRegions()[i], event.getIndexes()[i],
                              null, assembly,
                               ...
                               VSObjectFormatInfoModel origFormat, boolean reset)
   {
      ...
      if(tofmt != null) {
         ...
         if(reset) {
            textFormat.getFormat().setFormat(null);
            textFormat.getFormat().setFormatSpec(null);
          }
    ...
    */
   private RegionInfo getChartRegionFormat(String runtimeId,
                                           Principal principal, String name, String region,
    ...
    */
   private void getChartFormat(String runtimeId, GetVSObjectFormatEvent event, Principal principal,
      ...
      throws Exception
   {
      ...
      RegionInfo regionInfo = getChartRegionFormat(
         runtimeId, principal, event.getName(), event.getRegion(), event.getIndex(),
    ...
    */
   private void setChartFormat(String runtimeId, String region, int[] indexes, String[] columnNames,
      ...
      if(indexes == null || indexes.length == 0) {
         ...
         for(int i = 0; i < indexes.length; i++) {
               ...
               !"_Parallel_Label_".equals(columnName))
            {
               ...
               format = getChartRegionFormat(runtimeId, principal, chart.getAbsoluteName(), region,
                                             index, columnName, valueText).format;
            }
            ...
            if(format == null) {
               ...
               changeChartRegionFormat(runtimeId, principal, chart.getAbsoluteName(), region,
   ...
   private void changeChartRegionFormat(String runtimeId,
                                        Principal principal, String name, String region,
