...
public class VSWizardObjectController {
   ...
   @Autowired
   public VSWizardObjectController(VSTreeHandler treeHandler,
   ...
   public void openViewsheetWizardObject(OpenWizardObjectEvent event, Principal principal,
      ...
      throws Exception
   {
      RuntimeViewsheet rvs = viewsheetService.getViewsheet(event.getRuntimeId(), principal);
      ...
      throws Exception
   {
      ...
      try {
         ...
         /*
         dispatcher.sendCommand(new SwitchToMetaModeCommand());

         // other edit mode will go back to their original vs, so don't need to do destroy logic.
         if(VSWizardEditModes.WIZARD_DASHBOARD.equals(event.getOriginalMode())) {
            temporaryInfoService.destroyTemporary(id, principal);
