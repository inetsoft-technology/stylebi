...
public class AssistantProxyController {
   ...
   private static final Set<String> FORWARD_REQ_HEADERS = Set.of(
      "authorization", "content-type", "accept", "x-client-id", "x-request-id", "cookie");
      ...
      throws IOException
   {
      ...
      // Read body directly to avoid Spring's content-type restrictions on @RequestBody.
      // Cap at MAX_REQUEST_BODY_BYTES to prevent OOM from oversized payloads.
      ...
      if(internalBase == null || internalBase.trim().isEmpty()) {
         ...
         response.getOutputStream().write(
            "Proxy not configured: chat.app.internal.url is not set".getBytes(
      ...
      if(body != null) {
         // ContentType null — content-type already forwarded as a header above
         proxyRequest.setEntity(new ByteArrayEntity(body, null));
      }

      CloseableHttpClient client = proxiedPath.contains("/api/chat")
         ? getChatClient() : getDefaultClient();

      // internalBase is reassigned above (null-check fallback), so capture a final copy
      // for use inside the response-handler lambda.
      ...
      HttpClientResponseHandler<Void> handler = upstreamResponse -> {
         ...
         if(entity != null) {
            ...
            if(isHtml) {
               ...
               byte[] rewritten = html.getBytes(java.nio.charset.StandardCharsets.UTF_8);
      ...
      try {
         ...
         if(!response.isCommitted()) {
            ...
            response.getOutputStream().write(("Assistant unreachable: " + e.getMessage()).getBytes());
         }
      }
   }
   ...
   private static final String SSO_COMPLETE_HTML =
      ...
      "  var code = p.get('code'), origin = p.get('opener_origin');\n" +
      "  if (window.opener && origin) {\n" +
      "    window.opener.postMessage({type:'sso_complete', code:code}, origin);\n" +
