...
/**
 * Abstract chart controller base class to reduce boiler plate code necessary for
...
public abstract class VSChartController<T extends VSChartEvent> {
   ...
   {
      VSChartStateInfo chartState = getChartState(event, viewsheetService, principal);
   ...
   private VSChartStateInfo getChartState(
      T event, ViewsheetService viewsheetService, Principal principal) throws Exception
   {

      return VSChartStateInfo.createChartState(event, viewsheetService, principal, getRuntimeId());
   }
