...
/**
 * This class creates a XTable from a two-dimensional array.
 *
 * @version 7.0
 * @author InetSoft Technology Corp
 */
public class DefaultTable implements XTable {
   /**
    * Create a XTable from the array. The first row is treated as column header.
    */
   ...
   public Class getColType(int col) {
      Object obj = null;
      int i = getHeaderRowCount();
      int times = i + 1000;

      for(; i < times && moreRows(i); i++) {
         obj = getObject(i, col);
   ...
   /**
    * Clone the object.
    * @return the cloned object if successful, null otherwise.
    */
   @Override
   public Object clone() {
      try {
         DefaultTable table = (DefaultTable) super.clone();
         table.identifiers = (XIdentifierContainer) identifiers.clone();
         table.identifiers.setTable(table);
         return table;
      }
      catch(Exception ex) {
         LOG.error("Failed to clone DefaultTable", ex);
      }
   ...
   private static final Logger LOG =
      LoggerFactory.getLogger(DefaultTable.class);
}
