...
/**
 * XFloatColumn, maintains the meta information and data of one float table
 * column.
 *
 * @version 9.1
 * @author InetSoft Technology Corp
 */
public final class XFloatColumn extends AbstractTableColumn {
   /**
   ...
   /**
    * Create an instance of <tt>XFloatColumn</tt>.
    * @param isize the specified initial size.
    * @param size the specified max size.
    */
   public XFloatColumn(char isize, char size) {
      super();

      this.size = size;
      this.pos = 0;
      this.arr = new float[isize];
   }
   ...
   public XTableColumn addObject(Object obj) {
      ensureCapacity();
      arr[pos++] = obj == null ? Tool.NULL_FLOAT :
   ...
   public Object getObject(int r) {
      float[] arr = access();
      return arr[r] == Tool.NULL_FLOAT ? null : arr[r];
   }
   ...
   public ByteBuffer copyToBuffer() {
      ...
      buf.asFloatBuffer().put(arr, 0, pos);
