...
/**
 * XTimeColumn, maintains the meta information and data of one time table
 * column.
 *
 * @version 9.1
 * @author InetSoft Technology Corp
 */
public final class XTimeColumn extends AbstractTableColumn {
   /**
   ...
   /**
    * Create an instance of <tt>XTimeColumn</tt>.
    * @param isize the specified initial size.
    * @param size the specified max size.
    */
   public XTimeColumn(char isize, char size) {
      super();
   ...
   public Object getObject(int r) {
      long[] arr = access();
      return arr[r] == -1 ? null : new java.sql.Time(arr[r]);
   }
   ...
   private long[] arr;
   private char size;
   private static final Logger LOG = LoggerFactory.getLogger(XTimeColumn.class);
}
