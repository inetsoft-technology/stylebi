...
-- H2 SQL to produce inputs and expected output for JoinTableLensTest.Bug46758OuterJoin
...
CREATE ALIAS ORDER_YEAR AS $$
String orderYear(java.sql.Timestamp date) {
    return date == null ? null : Integer.toString(date.getYear() + 1900);
}
$$;

CREATE ALIAS ORDER_QUARTER_OF_YEAR AS $$
String orderQuarterOfYear(java.sql.Timestamp date) {
    if(date == null) {
        return null;
    }
    else if(date.before(new java.util.Date(date.getYear(), 3, 1))) {
        return "1st";
    }
    else if(date.before(new java.util.Date(date.getYear(), 6, 1))) {
        return "2nd";
    }
    else if(date.before(new java.util.Date(date.getYear(), 9, 1))) {
        return "3rd";
    }
...
CREATE ALIAS ORDER_QUARTER AS $$
String orderQuarter(java.sql.Timestamp date) {
    if(date == null) {
        return null;
    }
    ...
    if(date.before(new java.util.Date(date.getYear(), 3, 1))) {
        return year + "1st";
    }
    else if(date.before(new java.util.Date(date.getYear(), 6, 1))) {
        return year + "2nd";
...
CREATE ALIAS ORDER_MONTH_OF_YEAR AS $$
String orderMonthOfYear(java.sql.Timestamp date) {
    if(date == null) {
        return null;
    }

    return new java.text.SimpleDateFormat("MMM").format(date);
}
$$;
...
CREATE OR REPLACE VIEW OUTER_JOIN_TEST_LEFT AS
SELECT ORDER_YEAR(ORDERDATE)            AS "Year",
       ORDER_QUARTER_OF_YEAR(ORDERDATE) AS "QuarterOfYear",
       SUM(CUSTOMER_ID)                 AS "CustomerId"
FROM MVORDERS
GROUP BY "Year", "QuarterOfYear";

CALL CSVWRITE ('bug-46758-outer-left.csv', 'SELECT * FROM OUTER_JOIN_TEST_LEFT');

-- right table input

CREATE OR REPLACE VIEW OUTER_JOIN_TEST_RIGHT AS
SELECT ORDER_QUARTER(ORDERDATE)       AS "Quarter",
       ORDER_MONTH_OF_YEAR(ORDERDATE) AS "MonthOfYear",
       SUM(ORDERNO)                   AS "OrderNumber"
FROM MVORDERS
GROUP BY "Quarter", "MonthOfYear";

CALL CSVWRITE ('bug-46758-outer-right.csv', 'SELECT * FROM OUTER_JOIN_TEST_RIGHT');

-- expected output

CREATE OR REPLACE VIEW OUTER_JOIN_TEST AS (SELECT l.*, r.*
FROM (SELECT ORDER_YEAR(ORDERDATE)            AS "Year",
             ORDER_QUARTER_OF_YEAR(ORDERDATE) AS "QuarterOfYear",
             SUM(CUSTOMER_ID)                 AS "CustomerId"
      FROM MVORDERS
      GROUP BY "Year", "QuarterOfYear") AS l
         RIGHT OUTER JOIN
     (SELECT ORDER_QUARTER(ORDERDATE)       AS "Quarter",
             ORDER_MONTH_OF_YEAR(ORDERDATE) AS "MonthOfYear",
             SUM(ORDERNO)                   AS "OrderNumber"
      FROM MVORDERS
      GROUP BY "Quarter", "MonthOfYear") AS r
     ON l."Year" = r."Quarter");

CALL CSVWRITE ('bug-46758-outer-expected.csv', 'SELECT * FROM OUTER_JOIN_TEST');
