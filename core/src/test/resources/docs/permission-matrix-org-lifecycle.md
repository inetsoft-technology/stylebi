...
> ℹ️ **场景 5：克隆组织后，源组织的权限授权是独立的一份、不受新组织后续变动影响**——复制组织（`copyOrganization(replace=false)`）时，源组织自己的内容权限授权与新组织各自独立，用户在真实前端反复验证，clone 前后源组织的用户始终能正常访问被授权的资源。这一特性依赖生产环境 `FileAuthorizationProvider` 的存储读写语义：底层走 `Cluster.getReplicatedMap()`，生产环境背后是真正的 `IgniteCache`，Ignite 默认 `copyOnRead=true`（本项目 `IgniteCluster.getCacheConfiguration()` 也没有把它关掉）——每次 `get()` 都拿到一份反序列化出来的独立拷贝，改这份拷贝不会影响存储里已经落盘的那份。测试用的共享 `MockCluster` 背后是一个裸 `ConcurrentHashMap`，`get()` 返回的是存进去时的原始引用，没有这层保护，直接用它观察这个场景会看到"改新组织那份权限对象连带把源组织那份也改了"的失真结果，因此该场景的测试需要单独覆盖一个带 copy-on-read 语义的 Cluster 配置。详见下方场景 5 和"测试方法论"小节。
...
| 5 | 复制 orgA → 全新 org（`replace=false`）：新 org 的资源权限是否正确继承自 orgA | orgA 保留自己的权限，新 org 拿到一份独立的等价授权 | `[已落地]` `copy_replaceFalse_contentPermissionGrantIsCopiedToNewOrg`——驱动真实入口 `AbstractEditableAuthenticationProvider.copyOrganization(replace=false)`（不是挑权限相关的子方法单独调，因为这个场景问的正是"整条路径是否保留权限"）。测试类的 Spring 配置里用 `CopyOnReadClusterConfig` 覆盖了共享的 `MockCluster` bean（只在这一个测试类的上下文里生效，不影响其他测试），模拟生产环境 `IgniteCache` 默认的 `copyOnRead=true` 语义；在这个更贴近生产的 Cluster 实现下断言：新组织的 key 拿到一份独立的 `viewer READ` 授权，源组织自己 key 下的授权原样保留、不受影响。这个场景对 Cluster 实现的 copy-on-read 语义比较敏感，若改用共享的 `MockCluster`（裸 `ConcurrentHashMap`，`get()` 返回原始引用，没有 copy-on-read 保护）观察，会看到失真的"源组织权限被就地改动"的结果，详见上方 callout 和"测试方法论"小节 |
...
测试环境用的 `MockCluster.getReplicatedMap()` 背后就是一个裸的 `ConcurrentHashMap`，`get()` 返回的是存进去时的**原始 Java 对象引用**，没有这层保护。任何生产代码里"先 `getPermissions()` 拿出来改一改、再存到另一个 key 下"这种模式（`IdentityService.updateIdentityPermissions()` 就是这么写的），在 `MockCluster` 下会连带把原 key 的数据也改坏，但在生产环境完全不会发生——场景 5（克隆组织）正是对这个语义差异敏感的例子。
