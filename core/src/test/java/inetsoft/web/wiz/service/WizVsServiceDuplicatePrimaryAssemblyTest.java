 ...
 * create() path's own rebind sequence (uniqueAssemblyName + rebindAssembly + demote/promote).
 *
...
class WizVsServiceDuplicatePrimaryAssemblyTest {
   ...
   void allocatesASeparateUniqueNameOnASecondDuplicate() throws Exception {
      // Simulates a second copy-then-apply turn on the same viewsheet (the FIRST copy is still there,
      // unsaved) — must not collide with the first copy's name.
      Viewsheet vs = new Viewsheet();
      ChartVSAssembly source = new ChartVSAssembly(vs, "Chart1");
      source.setPrimary(true);
      vs.addAssembly(source);
      ...
      VSAssembly secondCopy = service.duplicatePrimaryAssembly(rvsOf(vs), source);
