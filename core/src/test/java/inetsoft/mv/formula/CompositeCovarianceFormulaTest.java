...
class CompositeCovarianceFormulaTest {

   private CompositeCovarianceFormula formula;

   @BeforeEach
   void setUp() {
      formula = new CompositeCovarianceFormula();
   }

   @Test
   void testReset() {
      formula.addValue(new double[]{10, 2, 3, 4});
   ...
   void testAddValueWithDoubleArray() {
      ...
      formula.addValue(new double[]{10, 2, 3, 4});
      assertFalse(formula.isNull());
      assertEquals(2.125, formula.getResult());
      assertEquals(2.125, formula.getDoubleResult());

      //cnt_c1 is zero
      formula.reset();
      formula.addValue(new double[]{10, 2, 3, 0});
      assertFalse(formula.isNull());
      assertNull(formula.getResult());
      assertEquals(0.0, formula.getDoubleResult());

      // Invalid double array (length < 4)
      formula.reset();
      formula.addValue(new double[]{10, 2, 3});
      assertTrue(formula.isNull());
   }
   ...
   void testAddValueWithObjectArray() {
      ...
      formula.reset();
      Object[] values1 = {10.0, 2, 3};
      formula.addValue(values1);
      assertFalse(formula.isNull());
      assertEquals(2.8888888888888893, formula.getResult());
      assertEquals(2.8888888888888893, formula.getDoubleResult());

      // Valid object array, string values
      Object[] stringValues = {"10.0", "2", "3.0", "4.0"};
      formula.reset();
      formula.addValue(stringValues);
      assertFalse(formula.isNull());
      assertEquals(2.125, formula.getResult());
      assertEquals(2.125, formula.getDoubleResult());
      ...
      formula.reset();
      Object[] invalidValues = {"invalid", 2.0, 3.0, 4.0};
   ...
   @Test
   void testClone() {
      CompositeCovarianceFormula clonedFormula = (CompositeCovarianceFormula) formula.clone();
   ...
   void testGetDisplayName() {
      assertEquals("CovarianceFormula", formula.getDisplayName());
   }

   @Test
   void testGetName() {
      assertEquals(XConstants.COVARIANCE_FORMULA, formula.getName());
   }
