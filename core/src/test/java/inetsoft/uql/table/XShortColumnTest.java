...
class XShortColumnTest {

   private XShortColumn column;

   @BeforeEach
   void setUp() {
      column = new XShortColumn((char) 10, (char) 20);
   }

   @Test
   void testGetCreator() {
      assertNotNull(XShortColumn.getCreator());
      assertEquals(Short.class, XShortColumn.getCreator().getColType());
   }

   @Test
   void testGetType() {
      assertEquals(XSchema.SHORT, column.getType());
   }
   ...
   void testAddGetObject() {
      column.addObject((short) 100);
      column.addObject(Tool.NULL_SHORT);

      assertEquals((short) 100, column.getObject(0));
   ...
   void testCopyToBuffer() {
      column.addObject((short) 100);
      column.addObject((short) 200);

      ByteBuffer buffer = column.copyToBuffer();

      assertNotNull(buffer);
      assertEquals(4, buffer.limit());
      assertEquals(100, buffer.asShortBuffer().get(0));
      assertEquals(200, buffer.asShortBuffer().get(1));
   }
}