...
/**
 * Bug #76852: {@code setRanges}/{@code setRangeColors} grew their backing arrays on a longer
 * script write but never shrank them on a shorter one, leaving stale trailing entries from a
 * prior longer write visible in the rendered gauge/thermometer/cylinder/sliding-scale.
 */
...
class RangeOutputVSAssemblyInfoTest {
   ...
   /**
    * Bug #76852 refute finding: once the setters independently shrink to the caller's exact
    * incoming length, ranges.length and colors.length can legitimately diverge (e.g. a script
    * writes N ranges but N-1 colors). Cylinder/Thermometer/SlidingScale/BulletGraph must render
    * the agreeing prefix, not throw or silently render nothing.
    */
   @Test
   ...
   void slidingScaleRendersPartiallyInsteadOfThrowingOnLengthMismatch() throws Exception {
      ...
      java.lang.reflect.Constructor<VSSlidingScale> ctor =
         VSSlidingScale.class.getDeclaredConstructor();
      ...
      java.lang.reflect.Field linePosition = VSSlidingScale.class.getDeclaredField("linePosition");
      linePosition.setAccessible(true);
      linePosition.set(slidingScale, new java.awt.geom.Point2D.Double(0, 0));
