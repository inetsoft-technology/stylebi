---
description: Log in to a StyleBI deployment. Usage - /stylebi-composer-chat:login <stylebi-url>
---

The user wants to log into a StyleBI deployment. Do this in ONE turn — the user does not need to confirm anything; they just log in via the browser while `login_complete` waits. If the user explicitly says "log in" or "connect", that instruction is enough on its own — act immediately. Do not call `status` first to decide whether logging in is necessary; `status` reflects whatever URL was used at the last login and can be stale, and re-litigating an explicit request against a stale status is exactly the kind of hesitation that stalls this flow.
