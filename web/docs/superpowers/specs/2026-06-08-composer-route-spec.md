# /composer 路由测试执行 Spec

**所属设计文档**: `docs/superpowers/specs/2026-06-08-portal-unit-test-strategy-design.md`
**路由入口**: `projects/portal/src/app/composer/composer.routes.ts`
**预估候选组件数**: 40–55 | **预估推进数**: 25–35
...
| 项目 | 状态 | 说明 |
|------|------|------|
| Pre-scan workflow | 需创建 | `.claude/workflows/portal-prescan.js`（见 plan 文件 Task 1） |
| Layer 1 handler 文件 | 已存在 | `mocks/handlers/composer.handlers.ts`，按需追加 |
...
读取 `projects/portal/src/app/composer/composer.routes.ts`，展开 1–2 层子组件，写入候选清单：

```
docs/superpowers/specs/test-prescan/composer-candidates.txt
```
...
输入 `composer-candidates.txt`，调用 `portal-prescan` workflow，输出报告：
...
读取组件源码中所有 HTTP URL，补充至 `mocks/handlers/composer.handlers.ts`（不改变已有 handler 行为）。
...
`composer-prescan-YYYY-MM-DD.md` 本身即为进度看板：
