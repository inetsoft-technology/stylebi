...
**路由入口**: `projects/portal/src/app/portal/portal.routes.ts`
**预估候选组件数**: 25–35 | **预估推进数**: 15–25
...
| Layer 1 handler 文件 | 已存在 | `mocks/handlers/portal.handlers.ts`（约 615 行），大部分 /portal API 已有默认 handler，按需追加 |
...
读取 `projects/portal/src/app/portal/portal.routes.ts`，展开 1–2 层子组件，写入候选清单：

```
docs/superpowers/specs/test-prescan/portal-route-candidates.txt
```
...
输入 `portal-route-candidates.txt`，调用 `portal-prescan` workflow，输出报告：

```
docs/superpowers/specs/test-prescan/portal-route-prescan-YYYY-MM-DD.md
```
...
读取组件源码中所有 HTTP URL，确认 `mocks/handlers/portal.handlers.ts` 中已覆盖。对缺失 URL 补充默认 handler（不改变已有 handler 行为）。
...
```bash
npx vitest run projects/portal/src/app/portal/path/to/ComponentName.tl.spec.ts
```
...
| `Unhandled request: GET */api/...` | 在 `portal.handlers.ts` 补充对应 Layer 1 handler |
...
`portal-route-prescan-YYYY-MM-DD.md` 本身即为进度看板：
...
| Layer 1 handler | `mocks/handlers/portal.handlers.ts` |
