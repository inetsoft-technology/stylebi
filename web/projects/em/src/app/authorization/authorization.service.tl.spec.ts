...
it("should propagate HTTP errors and leave the cache empty so the next call retries", async () => {
   ...
   // Note: `.rejects.toThrow()` is unreliable in Angular's Zone.js test environment
   // because Zone.js intercepts rejected promises before they reach Jest's handler.
   // Capture the error directly in the observable pipeline instead.
   let thrownError: unknown;
   await firstValueFrom(
      service.getPermissions("settings").pipe(
         catchError(err => { thrownError = err; return of(null); })
      )
   );
