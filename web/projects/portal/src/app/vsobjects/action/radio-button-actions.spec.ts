...
describe("RadioButtonActions", () => {
   const createModel = () => TestUtils.createMockVSRadioButtonModel("RadioButton1");

   it("check status of menu actions of radio button in composer", () => {
      const expectedMenu = [
         [
            { id: "radio-button properties", visible: true },
            { id: "radio-button show-format-pane", visible: true }
         ],
         [
      ...
      const actions = new RadioButtonActions(model, ComposerContextProviderFactory());
      const menuActions = actions.menuActions;

      expect(menuActions).toMatchSnapshot();
   });

   it("check status of menu actions of radio button in viewer and preview", () => {
      const expectedMenu = [
         [
            { id: "radio-button properties", visible: false },
            { id: "radio-button show-format-pane", visible: false }
         ],
         [
      ...
      const model = createModel();
      const actions1 = new RadioButtonActions(model, ViewerContextProviderFactory(false));
      ...
      const actions2 = new RadioButtonActions(model2, ViewerContextProviderFactory(true));
   ...
   it("should not display menu action when as data tip component", () => {
      ...
      const model = createModel();
      const actions = new RadioButtonActions(model, ViewerContextProviderFactory(false), false, null, dataTipService);
      const menuActions = actions.menuActions;
