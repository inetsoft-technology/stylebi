...
describe("UploadActions", () => {
   const createModel = () => TestUtils.createMockVSUploadModel("Upload1");

   it("check status of menu actions of upload in composer", () => {
      const expectedMenu = [
         [
            { id: "upload properties", visible: true },
            { id: "upload show-format-pane", visible: true }
         ],
         [
      ...
      const actions = new UploadActions(model, ComposerContextProviderFactory());
      const menuActions = actions.menuActions;

      expect(menuActions).toMatchSnapshot();
   });

   it("check status of menu actions of upload in viewer and preview", () => {
      const expectedMenu = [
         [
            { id: "upload properties", visible: false },
            { id: "upload show-format-pane", visible: false }
         ],
         [
      ...
      const model = createModel();
      const actions1 = new UploadActions(model, ViewerContextProviderFactory(false));
      ...
      const actions2 = new UploadActions(model2, ViewerContextProviderFactory(true));
   ...
   it("should not display menu action when as data tip component", () => {
      ...
      const model = createModel();
      const actions = new UploadActions(model, ViewerContextProviderFactory(false), false, null, dataTipService);
      const menuActions = actions.menuActions;
