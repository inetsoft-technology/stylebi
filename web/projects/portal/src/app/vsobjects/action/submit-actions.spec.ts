...
describe("SubmitActions", () => {
   const createModel = () => TestUtils.createMockVSSubmitModel("Submit1");

   it("check status of menu actions of submit in composer", () => {
      const expectedMenu = [
         [
            { id: "submit properties", visible: true },
            { id: "submit show-format-pane", visible: true }
         ],
         [
      ...
      const actions = new SubmitActions(model, ComposerContextProviderFactory());
      const menuActions = actions.menuActions;

      expect(menuActions).toMatchSnapshot();
   });

   it("check status of menu actions of submit in viewer and preview", () => {
      const expectedMenu = [
         [
            { id: "submit properties", visible: false },
            { id: "submit show-format-pane", visible: false }
         ],
         [
      ...
      const model = createModel();
      const actions1 = new SubmitActions(model, ViewerContextProviderFactory(false));
      ...
      const actions2 = new SubmitActions(model2, ViewerContextProviderFactory(true));
   ...
   it("should not display menu action when as data tip component", () => {
      ...
      const model = createModel();
      const actions = new SubmitActions(model, ViewerContextProviderFactory(false), false, null, dataTipService);
      const menuActions = actions.menuActions;
