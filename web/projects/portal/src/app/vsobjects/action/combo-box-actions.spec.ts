...
describe("ComboBoxActions", () => {
   const createModel = () => TestUtils.createMockVSComboBoxModel("ComboBox1");

   it("check status of menu actions of combo box in composer", () => {
      const expectedMenu = [
         [
            { id: "combo-box properties", visible: true },
            { id: "combo-box show-format-pane", visible: true }
         ],
         [
      ...
      const actions = new ComboBoxActions(model, ComposerContextProviderFactory());
      const menuActions = actions.menuActions;

      expect(menuActions).toMatchSnapshot();
   });

   it("check status of menu actions of combo box in viewer and preview", () => {
      const expectedMenu = [
         [
            { id: "combo-box properties", visible: false },
            { id: "combo-box show-format-pane", visible: false }
         ],
         [
      ...
      const model = createModel();
      const actions1 = new ComboBoxActions(model, ViewerContextProviderFactory(false));
      ...
      const actions2 = new ComboBoxActions(model2, new ContextProvider(false, false, true, false, false, false, false, false, false, false, false));
