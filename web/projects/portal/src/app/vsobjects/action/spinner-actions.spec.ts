...
describe("SpinnerActions", () => {
   const createModel = () => TestUtils.createMockVSNumericRangeModel("VSSpinner", "Spinner1");

   it("check status of menu actions of Spinner in composer", () => {
      const expectedMenu = [
         [
            { id: "spinner properties", visible: true },
            { id: "spinner show-format-pane", visible: true }
         ],
         [
      ...
      const actions = new SpinnerActions(model, ComposerContextProviderFactory());
      const menuActions = actions.menuActions;

      expect(menuActions).toMatchSnapshot();
   });

   it("check status of menu actions of Spinner in viewer and preview", () => {
      const expectedMenu = [
         [
            { id: "spinner properties", visible: false },
            { id: "spinner show-format-pane", visible: false }
         ],
         [
      ...
      const model = createModel();
      const actions1 = new SpinnerActions(model, ViewerContextProviderFactory(false));
      ...
      const actions2 = new SpinnerActions(model2, ViewerContextProviderFactory(true));
   ...
   it("should not display menu action when as data tip component", () => {
      ...
      const model = createModel();
      const actions = new SpinnerActions(model, ViewerContextProviderFactory(false), false, null, dataTipService);
      const menuActions = actions.menuActions;
