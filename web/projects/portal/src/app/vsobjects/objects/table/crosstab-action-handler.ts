...
export class CrosstabActionHandler extends AbstractActionHandler {
   ...
   showCrosstabDialog<D, M>(dialogType: Type<D>, modelUri: string,
                            ...
                            options: NgbModalOptions = {windowClass: "property-dialog-window"}): void
   {
      this.modelService.getModel(modelUri, params).toPromise().then(
         ...
         (error: any) => {
            //TODO handle error
            console.error("Failed to get crosstab property model: ", error);
         }
      );
   }
}
