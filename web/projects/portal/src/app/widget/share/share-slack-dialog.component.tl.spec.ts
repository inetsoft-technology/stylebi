...
async function renderComponent(opts: RenderOpts = {}) {
   const { fixture } = await render(ShareSlackDialog, {
...
describe("ShareSlackDialog — ngOnInit", () => {
...
describe("ShareSlackDialog — ok()", () => {
   ...
   it("should show error dialog and reset loading=false on HTTP error", async () => {
      ...
      expect(MessageDialog.lastMessage).toBe("Failed to post message to Slack.");
      expect(comp.loading).toBe(false);
   });
...
describe("ShareSlackDialog — cancel()", () => {
