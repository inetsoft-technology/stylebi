...
import { EmbedImageActions } from "./embed-image-actions";
...
import { EMBED_IMAGE_URL_MATCHER } from "./embed-image.routes";
...
@Component({
    imports: [ResizedDirective, VSImage, MiniToolbar, InteractContainerDirective],
...
export class EmbedImageComponent extends CommandProcessor implements OnInit, OnDestroy, AfterViewInit {
   @Input() url: string;
   appSize: Dimension;
   assemblySize: Dimension;
   vsInfo: ViewsheetInfo;
   vsObject: VSImageModel;
   ...
   ngOnInit(): void {
      if(this.url) {
         ...
         const result = EMBED_IMAGE_URL_MATCHER(segments);
   ...
   processAddVSObjectCommand(command: AddVSObjectCommand): void {
      ...
      if(this.vsObject) {
         this.vsObject = <VSImageModel>VSUtil.replaceObject(Tool.clone(this.vsObject), command.model);
      }
      else {
         this.vsObject = <VSImageModel>command.model;
      }
   ...
   processRefreshVSObjectCommand(command: RefreshVSObjectCommand): void {
      ...
      if(!this.vsObject) {
         this.vsObject = <VSImageModel> command.info;
      }
      else {
         this.vsObject = <VSImageModel>VSUtil.replaceObject(Tool.clone(this.vsObject), command.info);
      }
   ...
   onResize() {
      ...
      this.debounceService.debounce("embed-image-vs-resize" + this.runtimeId, () => {
         if(this.inputRuntimeId) {
            const refreshEvent: RefreshVsAssemblyEvent = {
               vsRuntimeId: this.runtimeId,
               assemblyName: this.assemblyName
