...
export class EmbedCrosstabComponent extends CommandProcessor implements OnInit, OnDestroy, AfterViewInit {
   ...
   ngAfterViewInit(): void {
      ...
      // handle dropdown in a dialog outside the bounds of the crosstab element
    ...
    * EmbedCrosstabActions) is clicked - so a consumer embedding <inetsoft-crosstab> as a plain
    * custom element (no Angular APIs available) can toggle showing it bigger. Deliberately does
    * not touch model.maxMode/openMaxMode()/closeMaxMode() - see the comment on the "crosstab
   ...
   processAddVSObjectCommand(command: AddVSObjectCommand): void {
      ...
      if(this.vsObject) {
         this.vsObject = <VSCrosstabModel>VSUtil.replaceObject(Tool.clone(this.vsObject), command.model);
      }
      else {
         this.vsObject = <VSCrosstabModel>command.model;
      }
   ...
   processRefreshVSObjectCommand(command: RefreshVSObjectCommand): void {
      ...
      if(!this.vsObject) {
         this.vsObject = <VSCrosstabModel> command.info;
      }
      else {
         this.vsObject = <VSCrosstabModel>VSUtil.replaceObject(Tool.clone(this.vsObject), command.info);
      }
   ...
   onResize() {
      ...
      this.debounceService.debounce("embed-crosstab-vs-resize" + this.runtimeId, () => {
         if(this.inputRuntimeId) {
            const refreshEvent: RefreshVsAssemblyEvent = {
               vsRuntimeId: this.runtimeId,
               assemblyName: this.assemblyName,
