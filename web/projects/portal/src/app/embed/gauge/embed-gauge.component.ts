...
import { EmbedGaugeActions } from "./embed-gauge-actions";
...
import { EMBED_GAUGE_URL_MATCHER } from "./embed-gauge.routes";
...
@Component({
    imports: [ResizedDirective, VSGauge, MiniToolbar, InteractContainerDirective],
    selector: "embed-gauge",
    templateUrl: "./embed-gauge.component.html",
    styleUrls: ["./embed-gauge.component.scss"],
    providers: [
...
export class EmbedGaugeComponent extends CommandProcessor implements OnInit, OnDestroy, AfterViewInit {
   @Input() url: string;
   appSize: Dimension;
   assemblySize: Dimension;
   vsInfo: ViewsheetInfo;
   vsObject: VSGaugeModel;
   ...
   ngOnInit(): void {
      if(this.url) {
         ...
         const result = EMBED_GAUGE_URL_MATCHER(segments);
   ...
   processAddVSObjectCommand(command: AddVSObjectCommand): void {
      ...
      if(this.vsObject) {
         this.vsObject = <VSGaugeModel>VSUtil.replaceObject(Tool.clone(this.vsObject), command.model);
      }
      else {
         this.vsObject = <VSGaugeModel>command.model;
      }
   ...
   processRefreshVSObjectCommand(command: RefreshVSObjectCommand): void {
      ...
      if(!this.vsObject) {
         this.vsObject = <VSGaugeModel> command.info;
      }
      else {
         this.vsObject = <VSGaugeModel>VSUtil.replaceObject(Tool.clone(this.vsObject), command.info);
      }
   ...
   onOpenContextMenu(event: MouseEvent) {
      ...
      // EmbedGaugeActions has no menu actions, so a right click would otherwise open an empty
   ...
   onResize() {
      ...
      this.debounceService.debounce("embed-gauge-vs-resize" + this.runtimeId, () => {
         if(this.inputRuntimeId) {
            const refreshEvent: RefreshVsAssemblyEvent = {
               vsRuntimeId: this.runtimeId,
               assemblyName: this.assemblyName,
