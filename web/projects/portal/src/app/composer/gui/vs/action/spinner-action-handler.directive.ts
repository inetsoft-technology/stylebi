...
import { SpinnerActions } from "../../../../vsobjects/action/spinner-actions";
import { VSSpinnerModel } from "../../../../vsobjects/model/vs-spinner-model";
...
export class SpinnerActionHandlerDirective extends AbstractActionHandler implements OnDestroy {
   ...
   @Input()
   set actions(value: SpinnerActions) {
      this.unsubscribe();
   ...
   @Output() onOpenFormatPane = new EventEmitter<VSSpinnerModel>();
   ...
   private handleEvent(event: AssemblyActionEvent<VSSpinnerModel>): void {
      switch(event.id) {
      case "spinner properties":
         this.showPropertyDialog();
         break;
      case "spinner edit-script":
         this.showPropertyDialog(true);
         break;
      case "spinner show-format-pane":
   ...
   private showPropertyDialog(openToScript: boolean = false): void {
      const modelUri: string = "../api/" + SPINNER_PROPERTY_URI +
      ...
      this.modelService.getModel(modelUri).subscribe((data: SpinnerPropertyDialogModel) => {
         ...
         const dialog: SpinnerPropertyDialog = this.showDialog(
            SpinnerPropertyDialog, options,
            (result: SpinnerPropertyDialogModel) => {
               const eventUri: string =
                  "/events/" + SPINNER_PROPERTY_URI + this.model.absoluteName;
               this.vsInfo.socketConnection.sendEvent(eventUri, result);
               this.model.absoluteName = result.spinnerGeneralPaneModel.generalPropPaneModel
