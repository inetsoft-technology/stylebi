...
import { ImageActions } from "../../../../vsobjects/action/image-actions";
...
const IMAGE_PROPERTY_URI: string = "composer/vs/image-property-dialog-model/";
...
@Directive({
   selector: "[cImageActionHandler]"
})
export class ImageActionHandlerDirective extends AbstractActionHandler implements OnDestroy {
   @Input() model: VSImageModel;
   @Input() vsInfo: Viewsheet;
   private subscription: Subscription;

   @Input()
   set actions(value: ImageActions) {
      this.unsubscribe();
   ...
   private handleEvent(event: AssemblyActionEvent<VSImageModel>): void {
      switch(event.id) {
      case "image properties":
         this.showPropertyDialog();
         break;
      case "image conditions":
   ...
   private showPropertyDialog(openToScript: boolean = false): void {
      const modelUri: string = "../api/" + IMAGE_PROPERTY_URI +
      ...
      const params = new HttpParams()
       ...
       this.modelService.getModel(modelUri).subscribe((data: ImagePropertyDialogModel) => {
         ...
         const dialog: ImagePropertyDialog = this.showDialog(
            ImagePropertyDialog, options,
            (result: ImagePropertyDialogModel) => {
               const eventUri: string =
                  "/events/" + IMAGE_PROPERTY_URI + this.model.absoluteName;
               this.vsInfo.socketConnection.sendEvent(eventUri, result);
               this.model.absoluteName = result.imageGeneralPaneModel.outputGeneralPaneModel
