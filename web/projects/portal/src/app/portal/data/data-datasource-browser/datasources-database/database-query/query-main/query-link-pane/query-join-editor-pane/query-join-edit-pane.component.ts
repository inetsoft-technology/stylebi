...
import {
   JoinEditPaneModel
} from "../../../../../../model/datasources/database/physical-model/graph/join-edit-pane-model";
   ...
   OnChanges, OnInit, AfterViewInit, AfterViewChecked, OnDestroy
{
   ...
   ngOnInit(): void {
      ...
      let name1: string = this.model.tables[0].name;
      let name2: string = this.model.tables[1].name;

      if(name1 == name2) {
         this.showTables.push(this.model.tables[1]);
      }
      else {
         this.showTables = this.model.tables;
      }
   }
