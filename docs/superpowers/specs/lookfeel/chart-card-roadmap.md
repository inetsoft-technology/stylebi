...
| Item | Source | Note |
|---|---|---|
| **§10.2 — the seamless in-lane strip** (the anchored toolbar and kebab draw no surface of their own; two glyph tones derived from the card background's luminance) | `Chart Card Spec v3.dc.html` §10.2, deferred to from §02 | **S–M, browser-only, added 2026-08-25 while scoping L″.** §02 defers the strip's whole appearance here, and the reason is structural rather than aesthetic: *"The card background is author-set, so the strip cannot carry a fixed treatment."* What ships today is exactly the *"white bordered box"* §10.2 replaces — `mini-toolbar.component.scss:25-33` gives `.mini-toolbar-container` a `--inet-shell-surface-default` fill, a 1px `--inet-default-border-color` border and a 6px radius, unconditionally, so the anchored in-lane strip draws it too. **Seamless applies to the in-lane strip only.** The overlaid strip on a title-hidden assembly keeps its surface (§03), and the nav bar keeps `--inet-shadow-low` deliberately, *"so it reads as deliberately floating rather than as a strip anchoring forgot"* (§02). **§10.2's stated precondition is wrong about this code** — it says *"the glyph is an `<img>` today, which color cannot tint"*, but `mini-toolbar.component.html:48,72` render `<i [class]="icon + ' icon-size-small'">`, an icon-class element. Confirm whether those resolve through the icon font (tintable by `color`) or a background-image (not) before planning; if the former, the blocker does not exist and this is cheaper than the spec claims. Recorded in [the corrections doc](./chart-card-source-doc-corrections.md). **Not part of L″** — L″ is geometry only, and the two share no code |
