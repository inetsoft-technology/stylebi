...
| ~~**Chart interior dark palette**~~ | `Chart card dark values - ticket.md` item 3 | **DONE 2026-09-02.** See the Done table and [the design](./2026-09-02-chart-interior-dark-chrome-design.md) |
...
| **Chart interior dark chrome** — six `PlotDescriptor` values seeded at creation: the data-label ink and the x/y gridline, facet, diagonal and quadrant line colours. The roadmap had this as M-L design-first on the grounds that `GDefaults` has no dark branch; that is true and irrelevant, because `VSChartChromeDefaults` intercepts first and the dark palette already existed. **One of the six was a default-on defect** — `yGridStyle` defaults to `THIN_LINE`, so a modern-dark chart drew near-white `#eeeeee` gridlines on a `#252428` card. **And the composer had been disagreeing with the canvas in both modes**: the Chart Line pane resolved these values at ten sites while the render path took them raw at seven, so the pane displayed `#E8E5DE` while the canvas drew `#eeeeee` — invisible in light, glaring in dark. Seeding closed it by construction and made those ten calls deletable. Light mode changed too, deliberately. **The design's own portability rationale turned out to be only half true, found while testing**: the five line colours seed a `CompositeValue` DEFAULT tier that does not travel in the asset the way the design assumed — `CompositeValue.toString()` writes the DEFAULT tier only when `saveDefault` is true, and all five `PlotDescriptor` line-colour fields use the two-arg constructor, so it stays false and a seeded gridline colour is lost on round trip. The data-label ink is unaffected — its `CompositeTextFormat` always writes its DEFAULT tier. Accepted as a documented limitation rather than fixed here; the design's own claim is corrected in place rather than left standing. **The nine-check manual browser matrix ran and passed on 2026-09-10**, so this is shipped and seen rather than only shipped — though note per §1.1's correction that its dark-gridline check would have passed before this change too, since the CSS tier already covered x/y/facet; the checks that prove new behaviour are the data-label ink ones. The cross-module `-Pcommunity,enterprise` build still could not be exercised — pre-existing skew unrelated to this work, enterprise `main` calls a community `License.Builder.formLicensed` this branch's community history does not carry — so a community-only `clean install -DskipTests` was substituted and passed. Design: [the dark chrome design](./2026-09-02-chart-interior-dark-chrome-design.md), including its "What the implementation found" section | *"seed the plot's structural line colours"* + *"seed the chart's data-label ink instead of writing it per render"* + *"read the chart's gridline colour from one stored value"* + *"pin the plot chrome's revert, re-seed and asset paths"* |
| **The dark active-border value.** `--inet-viz-active-border` was the one token whose dark-block entry re-pointed at the *light* primary accent rather than taking a dark value, so every assembly's focus outline drew `#E58A2A` on a dark card while the selected cell four lines away in the same stylesheet drew teal. Predates the whole track — it arrived in the initial commit, 2024-07-12 — and is not a regression from the dark pass or the palette re-tune. **Two of the recording ticket's three framing questions fell to measurement rather than taste**: matching the cell's teal would have collapsed the active/selected split `_viz-tokens.scss:103` documents as deliberate *and* fed a family this file ranks for retirement; and going neutral, which the palette-coordination decision test argues for in the abstract, still measures worse than the orange it replaces. The value is `var(--inet-primary-color-dark)`, `#C96F12`, the brand accent's existing deep step: ΔE **0.130**, clear of the design set's 0.106 line, at 4.23:1 on the card — against 0.079 for the `#E58A2A` it replaces, which is under the line. **Re-measured after the rebase onto the re-tuned palette, and the figures moved**: the earlier 0.155/0.119 pair and the neutral's 0.060 were taken against the pre-re-tune `Modern Dark` head-8, which this branch no longer carries. On the re-tuned head-8 (`#4FA5FF #FF8367 #49447D #2DEEC6 #AE41F5 #FFCB82 #FE3290 #9FEB28`) the nearest neighbour for both oranges is slot 2 `#FF8367`, and the slate `#94A3B8` now measures 0.121 rather than 0.060. The conclusion survives — `#C96F12` at 0.130 still separates best, and it is still the only candidate that both clears the line and follows a brand rebrand — but the neutral now clears the line too and loses on margin rather than decisively, which is a weaker form of the original argument and is recorded here rather than left reading as settled. Those are head-8 figures: at nine or more series both palettes splice a legacy tail whose slot 31, `#CC6600`, sits ΔE 0.019 away, which the design records as the one axis where this change is a regression and no hue escapes. **A dark accent has to go deeper, not lighter** — every `Modern Dark` member is high-lightness by design, so lightness is the axis that separates. The ticket also had the blast radius wrong in both directions: the class occurs **once** in the repo, not across composer and viewer (the composer solved this for its own handles long ago, at `editable-object-container.component.scss:42`), but its overlay div is a *preceding sibling* of the wrapper carrying `viz-modern`/`viz-dark`, so no dark token could reach it and the reachability half was unmentioned. Design: [the dark active-border value](./2026-09-11-dark-active-border-design.md) | *"Give the viz active border a dark value"* · *"Draw the assembly focus outline in the dark accent"* |
| **Chart mark selection on a dark card.** A second defect with the same symptom as the row above — the light brand orange drawn by a selection affordance on a dark card — in a mechanism the token work cannot reach, found while verifying it and fixed in the same branch. `.viz-modern .chart-object-canvas` named `--inet-primary-color` directly with no dark counterpart, and **a chart's selected marks are not DOM**: `ChartTool.drawRegions` (`chart-tool.ts:781`) reads the canvas element's computed `color` as its `fillStyle` and `borderColor` as its `strokeStyle`. So the affordance resolves through exactly one rule, and every custom property the dark scope redefines was reaching that canvas and being ignored, because nothing read one. Dark now takes the **selected** family, so a selected mark and a selected title agree — but its *ink* rather than its *border*, which naming argues against and measurement forces: the family's border `#2DD4BF` is the re-tuned palette's own teal `#2DEEC6` to within ΔE **0.072**, under the design set's 0.106 line, so a selection stroked with it would not read apart from the data underneath it. The ink `#A8EEF5` clears at 0.114 and is the highest contrast of the three at 11.91:1. Light keeps the accent deliberately: the same family's *light* border is 1.43:1 on a white plot. The fill is a new `--inet-viz-selected-fill-dark`, the ink at the 28% `--inet-focus-ring-color` uses, authored as a literal `rgba()` rather than mixed from the ink because `ctx.fillStyle` rejects the `oklab()`/`color()` form `getComputedStyle` returns for a `color-mix()` and drops it without raising. **The rule's position is load-bearing**: it ties the light rule on specificity and a dark assembly carries both classes, so only source order keeps dark from resolving to the light accent. Covers all five `.chart-object-canvas` surfaces — plot, axis, legend, title and the plot reference canvas. Design: [the second defect](./2026-09-11-dark-active-border-design.md) | *"Draw chart mark selection in the selected family, dark only"* |
...
- **Whether the teal selection family has an owner.** It is unchanged in `_viz-tokens.scss:51-53`, and v3
  ...
  **2026-09-11: still open, and now with two more dark consumers to migrate, not fewer.** The
  active/selected split held — the assembly focus outline deliberately did *not* move to this family,
  because it is an *active* affordance and `_viz-tokens.scss` keeps that on the accent on purpose.
  But the canvas-selection fix in the same branch moved chart mark selection **onto** the family in
  dark: it strokes `--inet-viz-selected-text` and added `--inet-viz-selected-fill-dark` beside it.
  Note the constraint that forced it, because it also binds whoever retires the family: the family's
  *border* `#2DD4BF` measures ΔE **0.072** from the re-tuned dark palette's own teal `#2DEEC6`
  (slot 4) — under the design set's 0.106 line, so a selection stroked with it does not separate from
  its own data. The ink `#A8EEF5` clears at 0.114. Only the ink was usable.
