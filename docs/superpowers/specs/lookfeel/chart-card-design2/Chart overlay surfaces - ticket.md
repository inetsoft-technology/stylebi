...
## 2026-08-11 sync — items 7, 8, 9, 11 are shipped in code, not just decided

Verified on `viz-updates`. `ChartTool.drawRegions()` now takes an `areaName` parameter, defines
`CHROME_AREAS`/`isChromeArea()` exactly matching the item 7 area list, and gates `strokeOnly` on
`GuiTool.isVizModern() && isChromeArea(areaName)` — item 7 and item 8's fill/stroke split is live. The
canvas-computed-style override (item 8/9's mechanism) is unchanged. `vs-annotation.component.scss` has the
`:host-context(.viz-modern) .vs-annotation__rectangle--selected { border: 2px solid var(--inet-primary-color); }`
rule exactly as decided (item 11, shell ticket), plus two bonus bindings not previously discussed: annotation
body text now follows `--inet-text-color` instead of hardcoded black, and `.annotition-button`'s bevel border
is retokenized. All gated on `.viz-modern`, so verification against a gate-off build still needs doing, but
the code matches the decision precisely — no drift to reconcile.

**New, not in the original ticket:** the tooltip directive/component gained a data-mark-anchored tail
(`tooltip-tail-placement.ts`, `chart-tail-config.ts`, SVG chrome in `tooltip.component.html/scss`) —
`showTail`/`tailAxis`/`tailAnchor` wired from `chart-area.component.ts` for `tooltipStyle === "CARD"` charts
with inline SVG. This is real design work beyond the "5 mechanical token bindings + type-ramp" scope items 1–6
described (see the sibling shell ticket) — it changes tooltip *positioning*, not just its palette. Needs its
own review; flagging here since it rides the same `widget__card-tooltip` class this ticket's item 4 discusses.

**Unaffected:** `vs-chart.component.scss` (item 13, the Resize Plot sliders) does not appear in the sync
diff — still exactly as described below, IE11 deletion and retokenizing both still outstanding.

**Shipped for chart-owned surfaces only — the second selection idiom is still live.**
`Visualization Widget Spec.dc.html` §07 and §09 record what items 7–9 and 11 did not reach. The viz token
layer carries its own selection family: `--inet-viz-selected-bg-modern: #DDF1F5`,
`--inet-viz-selected-text-modern: #123C44`, `--inet-viz-selected-border-modern: #BFDDE5`
(`scss/_viz-tokens.scss:51-53`), described in that file as "kept distinct from shell/Composer selection."
So a chart and a table side by side render selection in orange and teal — the multiple-idiom problem this
ticket's decision was taken to end, reinstated in a layer the ticket did not audit.

Narrower than it reads: `_viz-tokens.scss:37-38,93` already define `--inet-viz-selected-border` and
`--inet-viz-active-border` as `var(--inet-primary-color)`, so the borders match this ticket's stroke
today. **Only the fills diverge.** §07 retires the teal family and derives the fill from
`--inet-primary-color` at 28% flattened against the assembly background, carried in the §03 re-seed
rather than a sixth resolver — and it is export-affecting, so it needs the same manual pass. Treat items
7–9 and 11 as done for chart-owned surfaces and open for the server-rendered ones.
