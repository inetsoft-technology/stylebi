...
**2026-08-11 sync: sections 2, 3a, 3b and the first three slices of 3c have shipped** on `viz-updates` —
selection stroke/fill split, menu-actions reachability, the §06 height ladder, and the anchored kebab for
chart + table/crosstab/calc-table, and now selection list and tree as **kebab-only** (a new permanent
`AbstractVSActions.kebabOnly` capability, not a rollout stage) — all live code, gated on
`GuiTool.isVizModern()`. What remains open: **calendar and the selection container** (the container is
explicitly its own slice in the code's own comment, and Case 3 of the discussion doc is still unanswered),
item 13 (Resize Plot sliders, untouched), and everything in section 4/5 below. Details in each doc's own
sync note.
...
All code references verified against `inetsoft-technology/stylebi`, branch `viz-updates` (tree `52c127c128c3`
as of the latest sync). Slices 1–3 of the eight-assembly rollout have shipped on this branch — see each
section's own note below and `github.md`.

**Read first:** `README.md` (entry point) · `Chart Card Spec v3.dc.html` (the spec) ·
`Chart overlay surfaces - decided visuals.dc.html` (the three decided changes that are visual, drawn).
...
```
  ...
  D. Menu actions (item 8) ──→ E. §06 ladder ──→ F. Eight-assembly rollout (calendar, container left)
                                                   ▲
  M. Seed mark (sibling spec, Visualization Widget Spec §03) ─┘  should land first

  G. Chart type scale ───────→ H. Outlined text conversion (separate ticket, also reads the mark)

  I. Resize Plot sliders ────  independent
  J. Chart colour literals ──  independent
  K. Chart card dark values ─  independent, reconciliation not authoring
  N. Seeded-value reversibility ── consolidation, blocked on M landing
```

Five things are ordered: **C is a pair**, **D precedes E precedes F**, **G precedes H**, **M precedes F**
(and H), and within any ticket the deletions come before the retokenizing.

**M is not yet landed.** Slices 1–3 of the rollout (chart, table family, selection list/tree kebab-only)
shipped without it. Calendar and the container — the two slices still open — should wait for the mark: it
records whether an assembly's seeded chrome ran under gate-on or gate-off, and skipping it means those two
slices ship with no reversibility record. See `Anchoring beyond charts - discussion.md` and
`Seeded value reversibility - ticket.md`.
...
`Chart overlay surfaces - decided visuals.dc.html` §02 before writing any CSS. Adds
...
thing the decision was taken to end. Drawn in `…decided visuals.dc.html` §01.
...
**3c. Eight-assembly rollout** (spec §02, sixth source edit). **Shipped for six of eight** — chart, table,
crosstab, calc table (slices 1–2), selection list and tree as kebab-only (slice 3). **Calendar and the
selection container remain, and should wait for the seed mark (M) to land first** — see the dependency
picture above. Hide MiniToolbar moves to the menu **in the
...
discussion.md`:** the range slider is out (no `titleVisible`). The `titleRatio` fit test that document asks
for **no longer applies to selection list and tree** — slice 3 shipped them kebab-only, so only the ~40px
kebab pill has to fit their share of the lane — but it does apply to the container, the one family left with
both a ratio-split title and action buttons. Two things the shipped slices leave for whoever picks this up:
Case 2's ~56px *width* floor was never implemented (only the 32px height floor removes chrome, and a narrow
filter never reaches it), and the resting kebab already displaced the selection list's pending-alert icon
(`right: 48px` under the gate) — expect one such collision per family.
...
- **Resize Plot sliders** (chart ticket item 13). Delete the IE11 block **first** — provably dead, Angular
  21 cannot run there, roughly half the file — then apply `…decided visuals.dc.html` §03. Keep it a native
...
- **Chart card dark values** (`Chart card dark values - ticket.md`) — a reconciliation, not a blank page:
  the server already seeds dark card background/border/canvas/text; this spec's light-mode literals were
  never checked against them. Carries the card radius conflict (server seeds 12px, this spec binds 6px) as
  a shared blocker with item N below.
- **Seeded-value reversibility** (`Seeded value reversibility - ticket.md`) — consolidate three separate
  mechanisms that all answer "did the gate seed this, so may it take it back" onto the mark (M above).
  Blocked on M landing; do not delete the other two mechanisms before it does.
