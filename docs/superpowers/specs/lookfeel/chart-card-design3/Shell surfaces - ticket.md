...
**2026-08-11 sync note.** Items 1, 3, 4, 5, 6 (tooltip cleanup + CARD ramp) and item 11 (annotation
selection border) have shipped on `viz-updates`, matching the decisions below. **Item 12 is only half
done, despite looking touched** — a third `$stacking-order` entry landed but it's a ceiling, not a
registration; `date-tip-helper.ts` still assigns the scrim/pop-source z-indexes at runtime outside the
registry, so the underlying bug (data tips two orders of magnitude below dropdowns, patched with
`+99998`/`+99999` offsets) is still open. Re-check both files before treating item 12 as closed.

**Type:** cleanup / token binding, plus one decided visual change · **Branch verified:** `viz-updates`, originally `epic-74519` @ `c75c3fabdf64`
...
**Companion drawings:** `Chart overlay surfaces - decided visuals.dc.html` §02 renders the tooltip ramp at
...
## 3. Two radii are off the scale, and calendar isn't on the scale at all
...
**A different problem, found separately: calendar's radius isn't a token at all.** `vs-calendar.component.html` binds `border-radius` directly to `model.objectFormat.roundCorner` — an arbitrary per-object pixel value the user sets in the format dialog, not one of the five scale values above. Calendar is also absent from `VSObjectChromeDefaults.isCornerSeedTarget()`, so unlike table/chart/selection cards it never gets a seeded 12px radius either. This means calendar can't inherit the anchoring rollout's card treatment (`Anchoring beyond charts - discussion.md`) without a decision on whether `roundCorner` stays a free per-object value or moves onto the scale — worth resolving before calendar's slice of that rollout ships, not as a mechanical rebind here.
...
visuals.dc.html` §02 before implementing, and do the consumer inventory (rule 1) first. Item 12's z-index
