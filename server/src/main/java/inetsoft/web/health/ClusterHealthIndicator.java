/*
 * This file is part of StyleBI.
 * Copyright (C) 2026  InetSoft Technology
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 */
package inetsoft.web.health;

import inetsoft.util.health.ClusterHealthService;
import inetsoft.util.health.ClusterHealthStatus;
import org.springframework.boot.actuate.health.Health;
import org.springframework.boot.actuate.health.HealthIndicator;
import org.springframework.stereotype.Component;

/**
 * {@code ClusterHealthIndicator} reports the health of the cluster for Kubernetes
 * readiness probes. The cluster is considered healthy when the partition map
 * exchange is complete and the topology is stable.
 */
@Component
public class ClusterHealthIndicator implements HealthIndicator {
   public ClusterHealthIndicator() {
      this.service = ClusterHealthService.getInstance();
   }

   @Override
   public Health health() {
      ClusterHealthStatus status = service.getStatus();

      if(!status.isReady()) {
         return Health.down()
            .withDetail("message", status.getMessage())
            .build();
      }

      return Health.up()
         .withDetail("message", status.getMessage())
         .build();
   }

   private final ClusterHealthService service;
}
