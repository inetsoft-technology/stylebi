...
public class FileSystemHealthIndicator implements HealthIndicator {
   ...
   public Health health() {
      FileSystemStatus status = service.getStatus();

      if(status.isFileSystemDown()) {
         Map<String, Boolean> details = new HashMap<>();

         for(FileSystemState state : status.getFileSystems()) {
            details.put(state.getPath(), state.isAvailable());
         }
   ...
   private final FileSystemHealthService service;
}
